...
The idea is that you can add multiple middlewares adjusting the request.
...
The middleware returns a `MiddlewareFunction` which has the same signature as the input function `fn` but may contain different code.
Let's investigate the different properties of the `MiddlewareOptions`:

- The `fn` handed in is the returned function from the previous middleware.
...
A very simple middleware logs a 'Hello World' and leaves the original function unchanged.
...
Note that a middleware is **not** actually executing the original function.
...
Often you will use multiple small middlewares to add functionality to the request.
Assume you want to include a `timeout` and `fallback` middleware.
The implementation of the middlewares would look like:
...
For the timeout middleware this property is crucial because a timeout must run in parallel to the actual function.
This makes a chain of middlewares different from a (asynchronous) pipeline where the methods are executed after each other.
...
The initial function is a `GET` request to `example.com`.
...
For a list of default resilience middlewares, a timeout should be added first, then a circuit breaker followed by a retry.
