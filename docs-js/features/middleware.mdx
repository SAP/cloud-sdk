...
The timeout only counts the time for the actual execution of the original function _f<sub>0</sub>_.
...
The timeout would apply to the time for the original request plus additional time for a potential fallback call (see _g<sub>1</sub>_ and _g<sub>2</sub>_ in the picture above).
