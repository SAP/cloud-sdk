...
Fetching the CSRF token can be enabled or disabled via the `httpRequestOptions` parameter of `executeHttpRequest`:

```ts
{
  fetchCsrfToken: BOOLEAN_TRUE_OR_FALSE;
}
```
...
In this example, we can only do the request when `fetchCsrfToken` is set to `false` because the httpbin endpoint does not implement CSRF token fetching.
For an example with CSRF token fetching enabled see the [http-client.spec.ts](https://github.com/SAP-samples/cloud-sdk-js/blob/main/samples/http-client-examples/http-client.spec.ts) example.
...
// response.data.args contains { customParam: 'a/by', requestParam: 'a/by' }
```
...
// response.request.path contains myParam=custom-param
// response.data.args contains myParam: 'custom-param'
```
