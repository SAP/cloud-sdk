...
The [`compress()`](pathname:///api/v4/functions/sap-cloud-sdk_http-client.compress.html) middleware supports four [modes](pathname:///api/v4/interfaces/sap-cloud-sdk_http-client.RequestCompressionMiddlewareOptions.html#mode):

**`auto`**: Compresses payloads only if they exceed a size threshold (recommended, default).
...
**`always`**: Forces compression regardless of payload size.

```ts
middleware: [compress({ mode: 'always', compressOptions: { level: 1 } })];
```

**`header-only`**: Sets the `Content-Encoding` header without compressing.
...
**`never`**: Disables compression even if the middleware is included.
