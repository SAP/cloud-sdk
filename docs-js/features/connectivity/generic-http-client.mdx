...
Fetching the CSRF token can be enabled or disabled via the `httpRequestOptions.fetchCsrfToken` parameter of `executeHttpRequest`:
...
For an example with CSRF token fetching enabled see [http-client.spec.ts](https://github.com/SAP-samples/cloud-sdk-js/blob/main/samples/http-client-examples/http-client.spec.ts).
...
// response.data.args contains { param1: 'a/by', param2: 'y1' }
```
