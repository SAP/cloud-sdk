...
This guide explains how you execute arbitrary HTTP requests using the SAP Cloud SDK.
...
In case the OData client does not satisfy your requirements, the next thing you should look into is the HTTP client which is explained in this guide.
...
Fetching CSRF token is automatically enabled for all requests changing data e.g. `POST`, `DELETE` or `PATCH` requests.
