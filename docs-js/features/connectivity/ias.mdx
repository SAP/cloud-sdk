...
The `resource` specifies the dependency identified by its name or identifier configured in IAS, for details please refer to the [App2App Resources](#app2app-resources) section.
...
In multi-tenant scenarios, specify the request context using [`requestAs`](#current-tenant-and-provider-tenant-options) (defaults to `'current-tenant'`).
For scenarios requiring explicit tenant identifiers, see the [Multi-Tenant Support](#multi-tenant-support) section:
