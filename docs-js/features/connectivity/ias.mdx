...
Alternatively, the `clientId` property can be used to specify the provider application's client ID.
This may be necessary if there are multiple dependencies on the same provider application with different permissions, providing `clientId` will grant access to all dependencies associated with that provider.
The `tenantId` property is optional and can be used in multi-tenant scenarios to specify the provider tenant.
...
The provided JWT is used as an assertion for token exchange:
