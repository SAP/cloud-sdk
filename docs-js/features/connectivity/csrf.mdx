...
The SAP Cloud SDK fetches a CSRF token for non `GET` requests per default.
This is done no for all clients the SAP Cloud SDK provides:
...
- [disable OpenApi clients](../openapi/execute-request#skip-csrf-token-handling).

In order to retrieve a token the SAP cloud SDK does the following steps:

Makes a request using the resource URL adding a `/` in the end:

```
Request:             HEAD /reource/path/
Request Header:      X-CSRF-Token=fetch
```

If this returns a token execute the request with the token.
If not, make a second call without the `/` is tried:
...
:::note
The SAP Cloud SDK will discontinue to fetch CSRF tokens per default.
From version 4 on you will need to add the `csrf()` middleware to enable fetching.
