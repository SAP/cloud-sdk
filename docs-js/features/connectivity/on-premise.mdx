...
The SAP Cloud SDK provides helpful features to allow connecting to on-premise systems.
...
- If you are in a VPN from where the on-premise system is reachable, use an [environment destination](./destination.mdx) to access the system directly.
...
The SAP Cloud SDK adds the `x-sap-security-session: create` header for requests to destinations with authentication type "SamlAssertion" to create security sessions automatically.
