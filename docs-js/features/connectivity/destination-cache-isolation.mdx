...
The SAP Cloud SDK sets the cache expiration time depending on the [authentication flow](./destination.mdx#authentication-and-json-web-token-retrieval):

- If no token with a lifetime is involved e.g. `BasicAuthentication` or `ClientCertificateAuthentication` the cache expires after 5 minutes.
- If the destination contains an authorization token, the token lifetime is used for the expiration time.
...
- JWT without `user_id`: Isolation is set to `Tenant` and the value for the tenant is the `zid` property of the JWT.
