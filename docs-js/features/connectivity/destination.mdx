...
WorkflowDefinitionsApi.queryDefinitions().execute({destinationName: 'my-workflow-service'})
```
...
MyRequest.execute({ destinationName: 'my-service-name', serviceBindingTransformFn })
```
...
```ts
MyRequest.execute({ destinationName: 'myDestination', jwt: 'yourJWT', selectionStrategy: 'alwaysSubscriber' })
```
