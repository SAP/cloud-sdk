...
The SAP Cloud SDK uses a representation of a [destination](pathname:///api/v3/interfaces/sap_cloud_sdk_connectivity.Destination.html) to execute [arbitrary requests](../generic-http-client.mdx), [OData requests](../odata/overview.mdx), and [OpenAPI requests](../openapi/overview.mdx) as well as to [send e-mails](../mail-client/mail-client.mdx).

The SAP Cloud SDK helps you to authenticate against the destination service and retrieve destinations for your application.
...
`myDestination` is an SAP Cloud SDK representation of a [destination](pathname:///api/v3/interfaces/sap_cloud_sdk_connectivity.Destination.html).
The given request is executed against `myDestination`.
...
MyRequest.execute({ destinationName: 'DESTINATION' });
```
...
If you need the destination for another custom service, create a function with [ServiceBindingTransformFunction type](pathname:///api/v3/types/sap_cloud_sdk_connectivity.ServiceBindingTransformFunction.html) to transform the [Service interface](pathname:///api/v3/interfaces/sap_cloud_sdk_connectivity.Service.html) into the [Destination interface](pathname:///api/v3/interfaces/sap_cloud_sdk_connectivity.Destination.html).
...
If you want to consider only the service bindings, call the [destinationForServiceBinding()](pathname:///api/v3/functions/sap_cloud_sdk_connectivity.destinationForServiceBinding.html) function with the service name and options.
...
### Forwarding Auth Token

The SDK provides the option of configuring a destination to directly forward a user JWT to the target system.
This hebaiour can be enabled by setting the `forwardAuthToken` property on the destination to `true`, as shown in the code snippet below:
...
After configuring the destination, you can execute the request by calling `execute()` or `executeHttpRequest()` methods and passing the token you wish to forward, as shown below:

```ts
executeHttpRequest({ destinationName: 'myDestination', jwt: 'userJwt' });
```

In the above request, the authorization header with the specified JWT will always be sent, overriding the authentication type of the destination.
It is important to note that this approach will only work if the target system accepts unchanged JWTs.
If any transformation is required, e.g., from `OAuth` to `SamlBearer`, it is recommended to use the destination service.

:::note
If a custom authorization header is provided in the `requestConfig`, it will take precedence over the JWT to be forwarded.
:::
