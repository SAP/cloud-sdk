...
`destinations` environment variables are used for the destination lookup in the aforementioned order if they are presented.
The value of the enviroment variable is expected to be a stringified JSON array, where the items adhere to the <LinkToLatestJsApiDocs slug="interfaces/sap_cloud_sdk_connectivity.Destination" name="Destination interface" />.

Example:

```text
"[{\"name\": \"TESTINATION\", \"url\": \"http://url.hana.ondemand.com\", \"username\": \"DUMMY\", \"password\": \"dummy\"}]"
```
...
Methods such as `execute()` and `executeHttpRequest()` call `getDestination()` internally for the destination lookup.
...
The service credentials, in other words `VCAP_SERVICES` environment variables, represent services bound to the application.
...
you would call the service API using the instance name `my-workflow-service`:
...
Then pass it to `serviceBindingTransformFn` in `DestinationFetchOptions`.
...
The `execute()` or `getDestination()` try multiple steps to retrieve a destination as discussed in the beginning.
If you want to consider only the service bindings, call <LinkToLatestJsApiDocs slug="functions/sap_cloud_sdk_connectivity.destinationForServiceBinding" name="destinationForServiceBinding" /> function with the service name and options.
...
This is possible through the `getAllDestinationsFromDestionService` function.
Based on the provided JWT, you will either receive all subscriber or provider destinations.

Example:
```ts
import { getAllDestinationsFromDestionService } from '@sap-cloud-sdk/connectivity';

// Will attempt to get all provider destinations
getAllDestinationsFromDestionService();

// Will attempt to get all subscriber destinations
getAllDestinationsFromDestionService(subscriberJWT);
```

It's important to note that these destinations won't contain an authentication token.
