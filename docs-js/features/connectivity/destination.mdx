...
```ts
.execute({ destinationName: 'service-name' })
```

Currently, 5 services are supported out of the box:
...
If you need the destination for another custom service, create a function with type <LinkToLatestJsApiDocs slug="interfaces/sap_cloud_sdk_connectivity.ServiceBindingTransformFunction" name="ServiceBindingTransformFunction" /> to transform the <LinkToLatestJsApiDocs slug="interfaces/sap_cloud_sdk_connectivity.ServiceBinding" name="ServiceBinding interface" /> into the <LinkToLatestJsApiDocs slug="interfaces/sap_cloud_sdk_connectivity.Destination" name="Destination interface" />.
...
.execute({ destinationName: 'service-name', serviceBindingTransformFn })
```
...
To search the destination explicitly from service bindings, call <LinkToLatestJsApiDocs slug="interfaces/sap_cloud_sdk_connectivity.destinationForServiceBinding" name="destinationForServiceBinding" /> function with the service name and options.

For example:

```ts
const destination = destinationForServiceBinding('service-name', {
