...
Note that the `registerDestination()` function is tenant-aware and uses caching by default.
...
The caching of mTLS certificates is enabled by default, but can be disabled by adding the `useCache: false` option:
