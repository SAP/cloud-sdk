...
The minimal requirement towards a manually constructed destination is that you provide the `url` property.
...
The `destinations` environment variables are used for the destination lookup in the aforementioned order if they are presented.
...
Functions such as `execute()` and `executeHttpRequest()` call `getDestination()` internally for the destination lookup.
...
The service credentials, in other words the `VCAP_SERVICES` environment variables, represent services bound to the application.
...
Different selection strategies are defined in the SAP Cloud SDK:
...
Replace `HEADER_KEY` and `QUERY_KEY` with the name of your headers or query parameters and set the respective values.
