...
In addition to the default internal transformations, the SAP Cloud SDK provides the `getServiceBinding` function to retrieve a service type from a string input and a public convenience function `transformServiceBindingToDestination` that simplifies the process of transforming a service binding to a destination.
...
The function supports the [default service types](#service-binding-environment-variables) as listed above.
