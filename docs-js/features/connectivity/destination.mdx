...
```ts
MyRequest.execute({ destinationName: 'service-name' })
```
...
// replace 'service-name' with one of the services above
MyRequest.execute({ destinationName: 'service-name', serviceBindingTransformFn })
```
...
To search the destination explicitly for service bindings, call <LinkToLatestJsApiDocs slug="functions/sap_cloud_sdk_connectivity.destinationForServiceBinding" name="destinationForServiceBinding" /> function with the service name and options.
