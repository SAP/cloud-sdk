...
myEntityApi
  .requestBuilder()
  .getAll()
  .setBasePath('my/custom/service/path');
```

This chagne the base path of your request.
Executing the example request above against a destination with the URL `https://my.s4-system.com` would result in a request against the target like this: `https://my.s4-system.com/my/custom/service/path/MyEntity`.
