...
A changeset is a collection of HTTP `POST`, `PUT`, `PATCH` and `DELETE` operations - requests built by [`CreateRequestBuilder`s](#create-request-builder), [`UpdateRequestBuilder`s](#update-request-builder), [`DeleteRequestBuilder`s](#delete-request-builder) or [`ActionImportRequestBuilder`](#action-imports) in terms of the SAP Cloud SDK.
