...
If you need to adjust the way CSRF tokens are fetched, you can do so by using [middlewares](../middleware):
...
In terms of the SAP Cloud SDK this includes all requests built by [`GetAllRequestBuilder`](#getall-request-builder), [`GetByKeyRequestBuilder`](#getbykey-request-builder) and [`OperationRequestBuilder`](#operation-request-builder).
...
Such operations are built by the [`CreateRequestBuilder`s](#create-request-builder), [`UpdateRequestBuilder`s](#update-request-builder), [`DeleteRequestBuilder`s](#delete-request-builder) or [`OperationRequestBuilder`](#operation-request-builder).
