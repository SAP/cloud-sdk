...
- `basePath`: the URL path to be prepended before every request (e.g. `'/sap/opu/odata/sap/'` for SAP S/4HANA services).
  ...
  However, if automatic determination fails, the SDK will set the `basePath` value to `/` and log a warning message. You can also manually specify the `basePath` in the `options-per-service.json` file and re-generate the client.
