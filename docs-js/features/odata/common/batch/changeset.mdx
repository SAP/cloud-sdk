...
In the example below, we create a list of [`UpdateRequestBuilder` instances](#update-request-builder) from a list of addresses.
...
Once we execute the batch request, we get a list of `BatchResponse` instances, which in this example contains one response only, i.e. the one for the changeset.
