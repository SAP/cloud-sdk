...
As function imports are not entity based, you obtain the response using the `responseTransformer()` method, which is part of the `FunctionImportRequestBuilder`:
