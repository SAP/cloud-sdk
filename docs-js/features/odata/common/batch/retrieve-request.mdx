...
This service is a OData v2 service, but the syntax is the same for OData v2 and v4.
...
async function batchExample(documentIds: string[]): Promise<void> {
  const getByKeyRequests = documentIds.map(billingDocument =>
    billingDocumentApi.requestBuilder().getByKey(billingDocument)
  );
  const functionImportRequest = documentIds.map(billingDocument =>
