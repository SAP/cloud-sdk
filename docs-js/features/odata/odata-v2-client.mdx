...
For more details on **how to execute requests** using a OData type-safe client by the SAP Cloud SDK, refer to [this guide](./execute-odata-request).
...
A changeset is a collection of HTTP `POST`, `PUT`, `PATCH` and `DELETE` operations - requests built by [`CreateRequestBuilder`s](#create-request-builder), [`UpdateRequestBuilder`s](#update-request-builder), [`DeleteRequestBuilder`s](#delete-request-builder) or [`FunctionImportRequestBuilder`](#function-imports) using the mentioned HTTP operations in terms of the SAP Cloud SDK.
