...
This differs from the OData generator, which currently creates all sources needed for an npm package and includes transpilation.
...
Note that this formatting is done in memory before the files are emitted by the generator so no expensive additional I/O is required.
Alternatively, you can execute a totally custom formatting step after the generation is finished.
