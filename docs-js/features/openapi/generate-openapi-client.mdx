...
Please note that custom formatting steps could break source maps when you generate a JavaScript client (option `generateJs` enabled).
We recommend to exclude JavaScript clients from formatting because the generated `.js` and `.map.js` files are not meant for humans to be read.
