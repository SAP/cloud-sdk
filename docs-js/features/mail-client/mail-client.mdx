...
Set `debug` and `logger` options to `true` to log all the data that is passed to the server in the console. If `logger` is not set or set to `false`, nothing gets logged.
