...
The string argument in the `createLogger()` function is the identifier for the logger and you should use a separate message context for each logical part of your application.
...
The log level can be set using the environment variable `SAP_CLOUD_SDK_LOG_LEVEL`.
...
Via the `container` class, the SAP Cloud SDK provides methods per message context:
...
In case you see a need for higher flexibility of the logging instance feel free to create an issue in the [SAP Cloud SDK repository](https://github.com/SAP/cloud-sdk-js) or make a contribution.
