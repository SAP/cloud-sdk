...
Depending on the type of property, there can be additional functions like `greaterThan` or `greaterOrEqual`.
...
For more complicated filter conditions have a look at the [filter documentation](../../features/odata/odata-v2-client#filter).
...
If you call the URL `http://localhost:8080/business-partner/<yourId>`, the service returns a single business partner with the selected address information:
