...
This guide focuses on points where the SAP Cloud SDK team saw the need of a more detailed guide with samples.
The selection is related to support issues appearing in the past and therefore it is not a complete guide.
...
Let's investigate the implementation of `service-endpoint.ts` above the service will try to obtain a JSON web token (JWT) from the request.
...
The details would be too much for this guide and you may have a look at [our approuter guide](../guides/how-to-use-the-approuter.mdx).
...
- In our simple scenario, the `xs-app.json` file defines only one route consisting of a `source`, `target`, and `destination`.
  ...
  In our example `https://route-prefix-YOUR-SUBDOMAIN.cfapps.YOUR_REGION.hana.ondemand.com/SOME_VALUE` will lead to `SOME_VALUE` as the capturing group and `SOME_VALUE` is attached to the destination.
...
- The `manifest.yml` defines the available destinations for the approuter.
  ...
  Therefore `https://route-prefix-YOUR-SUBDOMAIN.cfapps.YOUR_REGION.hana.ondemand.com/SOME_VALUE` goes to `https://multi-tenant-app.cfapps.YOUR_REGION.hana.ondemand.com/SOME_VALUE`
