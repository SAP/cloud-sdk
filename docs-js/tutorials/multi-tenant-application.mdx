...
The code discussed in this guide can be found on the [samples repository](https://github.com/SAP-samples/cloud-sdk-js/tree/main/samples/cf-multi-tenant-application).
...
The code in example is not a copy-paste solution for productive use, but a didactic sample.
...
On SAP BTP different organization are modelled by subaccounts and multi-tenant application are subscribed per subaccount.
These organizations are potentially different companies or strongly separated parts of one company.
In any case, each organization has its subaccount on SAP BTP and subscribes to your multi-tenant application.
...
:::note
This is not a complete guide on the multi-tenancy topic in SAP, but rather covers only points where the SAP Cloud SDK team saw the need of a more detailed guide with samples.
...
You find the application code in the [multi-tenant-app folder](https://github.com/SAP-samples/cloud-sdk-js/tree/main/samples/cf-multi-tenant-application).
...
- In the `application.ts` file, the different endpoints are defined.
  For now, only the `/service` endpoint is relevant, which represents our multi-tenant service.
- In the `manifest.yml` file, the route to the application is given and the used services are defined.
- In the `service-endpoint.ts` a tenant-aware service (destination service) is called and tenant information are collected.
...
- Fetch a destination with the name `myDestination` using the destination service.
...
:::note
More information on the app router topic can be found in [this guide](../guides/how-to-use-the-approuter.mdx).
:::
...
You find all files in the [approuter folder](https://github.com/SAP-samples/cloud-sdk-js/tree/main/samples/cf-multi-tenant-application/approuter).
...
```
GET https://route-prefix-YOUR-SUBDOMAIN.cfapps.YOUR_REGION.hana.ondemand.com/
```
...
- In our simple scenario, the `xs-app.json` file defines only one route consisting of a `source`, `target`, and `destination`.
  The `source` is a regex and the target defines which capturing group is used in the destination.
  In our example `https://route-prefix-YOUR_SUBDOMAIN.cfapps.YOUR_REGION.hana.ondemand.com/SOME_VALUE` will lead to `SOME_VALUE` as the capturing group and `SOME_VALUE` is attached to the destination.
...
- The `manifest.yml` defines the available destinations for the approuter.
  The destination `multi-tenant-app` points to the URL of our application.
...
```
GET https://route-prefix-YOUR-SUBDOMAIN.cfapps.YOUR_REGION.hana.ondemand.com/service
```
...
You find the `saas-registry-config.json` in the [samples repository](https://github.com/SAP-samples/cloud-sdk-js/tree/main/samples/cf-multi-tenant-application).
...
Remember to add all SAP BTP services used by your application in the response of the [dependencies endpoint](https://github.com/SAP-samples/cloud-sdk-js/blob/2b31c670f6bcd4b89a9d1c8b88fd20974d1d5470/samples/cf-multi-tenant-application/multi-tenant-app/src/dependencies-endpoint.ts#L4).
...
After you created the instance of the `SAAS provisioning service` in the provider account, you can create a subscription via the user interface.
...
Hence, a script in the `subscription-endpoint.ts` creates a route automatically:
...
The code assume a destination with the name `cf-api` in the sample implementation which contains the access data for the CF API:
...
Log into your second SAP BTP account. Go to **Service->Instances and Subscriptions** and create a subscription to the `multi-tenant-app`.
...
The code will remove the route from the app router and make the application not reachable anymore for that user.
...
This chapter tries to elaborate a bit more how an actual multi-tenant application would look like and how the SAP Cloud SDK helps you.
