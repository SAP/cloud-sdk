...
- Install [`docker`](https://docs.docker.com/get-docker/) and a publicly reachable Docker repository.
  :::info
...
It will use your cluster's IP address and the port your application listens on.
...
You can see in the `spec.tls.hosts` part that it exposes 2 domains.
...
3. Create and bind the connectivity service with the `connectivty_proxy` plan.
   ...
   Additionally, to bind the secret represented by Kubernetes native YAML format, you need to convert it to JSON for the connectivity proxy to consume it.
   ...
   Then save the JSON as a secret.
   Here is an example:
...
1. Build the container image and deploy it into the container registry.
2. Re-start the Kubernetes Deployment.
