...
The [samples repository](https://github.com/SAP-samples/cloud-sdk-js/tree/main/samples/resilience-examples) gives you examples which illustrate the concepts and pick some widely used `npm` packages.

There is one resilience concept that is applied by default in the SAP Cloud SDK for JavaScript.
All calls going to SAP BTP services like XSUAA and the destination service have a circuit-breaker.
If need be, you can disable it via the `enableCircuitBreaker` option on the `execute()` methods.
...
- Consistent errors should not trigger a retry.
  For example, an HTTP request failing with 401 should not trigger a retry because the user is unauthorized.
- The number of retries should be limited to a low digit number.
  There should be some appropriate waiting time between retries.
