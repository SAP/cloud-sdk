...
Version 3.0 of the SDK introduced the concept of middlewares which is general and not resilience specific.
...
Note that [middlewares](../features/middleware) and therefore resilience applies to the request against the target system and per default nothing is added.
The SAP Cloud SDK applies resilience to the internal BTP service calls which are done under the hood.
...
```ts
import { resileince } from '@sap-cloud-sdk/resilience';

const [timeout, circuitBreaker] = resilience();
```

You can pass very basic options to adjust the created middlewares:
...
To use the `resilience()` function is the recommended way to build the resilience related middleware.
Using this method ensure the correct order of middlewares.

### Timeout

The timeout middleware is very simple and based on `Promise.race()`.
So no additional library is used.
You can create a timeout middleware with the `timeout()` function.
Per default a timeout of 10 seconds is used, but you can set values to change the value:
...
- The circuit breaker records all request for a duration of 10 seconds.
...
- After a reset time of 30 seconds the circuit breaker will go to the state half open.
...
This state is stored in central circuit breaker instates.
...
This means the maximum waiting time between the three retries are 2, 4 and 8 seconds.
...
Note that the concept of resilience is not trivial, and you should develop and test your implementation carefully:

- You should keep in mind the order of your middlewares.
  A retry should be in the end and a timeout or cache implementation in the very beginning.
...
- If your middleware adds shared state between requests e.g. circuit breaker you must consider isolation between customers in a multi tenant situation.
