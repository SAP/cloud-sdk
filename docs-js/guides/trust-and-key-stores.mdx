...
:::caution
The SAP Cloud SDK for JavaScript supports only `.pem` as certificate format for the truststore.
:::
...
:::caution
The SAP Cloud SDK for JavaScript only supports formats `.p12`, `.pfx`, `.pem` and `.jks` for the keystore.
:::
