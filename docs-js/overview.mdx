...
| Operates dynamically on the metadata and creates an on-the-fly-client. | Typed API clients can be [generated](features/odata/generate-client). An [generic, untyped client](features/connectivity/http-client) is also available. |
| Connectivity needs to be handled manually.                             | The SAP Cloud SDK handles [connectivity](features/connectivity/destinations) on the SAP BTP for typed and untyped API clients.                           |
