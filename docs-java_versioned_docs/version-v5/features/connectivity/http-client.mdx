...
To make use the `Apache5HttpClientAccessor`, make sure to include the [cloudplatform-connectivity](https://search.maven.org/search?q=g:com.sap.cloud.sdk.cloudplatform%2BAND%2Ba:cloudplatform-connectivity) **and** the [apache5-httpclient](https://search.maven.org/search?q=g:com.sap.cloud.sdk.frameworks%2BAND%2Ba:apache5-httpclient) dependencies in your project.

```java
HttpClient client = Apache5HttpClientAccessor.getHttpClient();
```
...
```java
HttpDestination destination = DestinationAccessor.getDestination("my-destination").asHttp();
HttpClient client = Apache5HttpClientAccessor.getHttpClient(destination);
```
...
```java
Apache5HttpClientFactory customFactory = new Apache5HttpClientFactoryBuilder()
...
The builder class (`Apache5HttpClientFactoryBuilder`) provides the following customization options:
...
2. `int maxConnectionsTotal` - The maximum number of parallel connections that can be established with a `HttpClient` created by the returned `HttpClient5Factory`.
...
If these customization options are not enough, consider creating your own implementation of the `Apache5HttpClientFactory`.
...
```java
Apache5HttpClientFactory factory = new MyCustomFactory();
Apache5HttpClientAccessor.setHttpClientFactory(factory);
```
