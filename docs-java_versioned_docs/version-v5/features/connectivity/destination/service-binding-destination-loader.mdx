...
In contrast to the `DestinationLoader` API, however, the `ServiceBindingDestinationLoader` API requires more sophisticated input, namely the `ServiceBindingDestinationOptions`.
