...
The first implementation use an `enum` to implement a **fixed set of choices** option.
...
The second implementation should allow the user to provide an arbitrary HTTP header value.
Therefore, using an `enum` as the parameter type would not be a good idea.
...
if (maybeOption.isEmpty()) {
    return; // we are not handling this service binding
}
