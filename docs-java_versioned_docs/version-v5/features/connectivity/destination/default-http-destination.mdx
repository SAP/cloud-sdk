...
5. (Conditional) Headers that are dervied from [authentication related properties](#deriving-the-authorization-header), **iff** no `Authorization` header was provided by any of the previous options.
6. (Conditional) Headers that are derived from [proxy authentication related properties](#deriving-the-proxy-authorization-header), **iff** no `Proxy-Authorization` header was provided by any of the previous options.

### On-Premise Handling

The `DefaultHttpDestination` implements dedicated support for on-premise connectivity.

This feature is enabled by setting the `ProxyType` property to `OnPremise`.
Doing so will trigger the following behavior:

* The `Proxy-Authorization` header will be derived from the bound [SAP BTP Connectivity Service on Cloud Foundry](https://help.sap.com/docs/connectivity/sap-btp-connectivity-cf/what-is-sap-btp-connectivity) as [explained here](service-binding-destination-loader#dealing-with-arbitrary-service-bindings).
* The `SAP-Connectivity-SCC-Location_ID` header will be added, **iff** the `CloudConnectorLocationId` property is set.

#### Caution

The `DefaultHttpDestination` will throw an exception if the `ProxyType` is set to `OnPremise` and there are multiple or no service bindings for the connectivity service.
...
We encourage users to rather use [the `DestinationAccessor` API](destination-accessor) or [the `ServiceBindingDestinationLoader` API](service-binding-destination-loader) to retrieve destinations.

:::

The `DefaultHttpDestination.Builder` allows you to create a `DefaultHttpDestination` instance from scratch.
...
This way is implemented in the `static fromDestination(Destination)` and `static fromProperties(DestinationProperties)` methods.
...
In the following example, we are retrieving a destination using [the `DestinationAccessor` API](destination-accessor).
...
Individual headers can be added via the `DefaultHttpDestination.Builder#header(Header)` and `DefaultHttpDestination.Builder#header(String, String)` methods.
...
The `DefaultHttpDestination.Builder#headerProviders(DestinationHeaderProvider...)` method allows you to add as many `DestinationHeaderProvider`s to the `DefaultHttpDestination` instance as you wish.
...
The `ErpDestinationHeaderProvider` is part of the [`cloudplatform-connectivity` artifact](https://search.maven.org/artifact/com.sap.cloud.sdk.cloudplatform/cloudplatform-connectivity) and, therefore, will **always** be added to any `DefaultHttpDestination` instance. <br />
