...
:::warning RFC, JCo and WAR Deployment
`RFC` and `JCo` modules, and, WAR Deployment are **not yet supported**.
They will only be available after the release of SAP Java Buildpack 2.
For now, only SAP Cloud SDK version 4 supports `RFC`, `JCo` and WAR Deployment.
:::
