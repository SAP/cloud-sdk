...
These BOMs should be listed before any SAP Cloud SDK related BOM, so for Spring Boot your `dependencyManagement` section should look something like this:
...
At the same time you should also replace usages of `javax` dependencies with the respective `jakarta` dependency. For example:
...
- Module `com.sap.cloud.sdk.cloudplatform:scp-cf` has been removed and functionally replaced by `com.sap.cloud.sdk:sdk-core`
