...
The REST generator is a wrapper around the public open-source [OpenAPI Generator](https://github.com/OpenAPITools/openapi-generator) where we have adjusted the `mustache` templates to integrate the generated client library with the SDK core.
...
Note the phase "generate-sources" is just a recommendation from our side, it can be changed as per your project's requirement.
Refer the [Maven build lifecycle guide](https://maven.apache.org/guides/introduction/introduction-to-the-lifecycle.html#Lifecycle_Reference) for complete explanation.
