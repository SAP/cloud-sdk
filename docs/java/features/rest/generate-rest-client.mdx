...
In order to use the generated client library either SDK BOM should be part of your project's dependency management section or the version for the rest-core artifact must be mentioned explicitly here.

:::note
Please note that the REST generator is not yet recommended for productive usage. It is more like an experimental preview from Cloud SDK.
:::

## Using the REST generator Maven plugin ##

To use the maven plugin following xml should be added to the `application/pom.xml` file of your project: 
