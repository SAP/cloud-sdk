...
You can influence the service class name and the methods by adding extension fields to the OpenApi specification.
Thereby you can leave the `tag` and `operationId` fields untouched if they need to stay stable.

The Java class name can be overriden with the extension field `x-sap-cloud-sdk-api-name`.
