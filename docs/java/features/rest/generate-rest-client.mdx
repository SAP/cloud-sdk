...
## Influence Java Class and Method Names
...
:::tip Removal of `api` Suffix
If the `x-sap-cloud-sdk-api-name` field value ends on `api` (regardless of the case), this suffix gets removed by the OpenAPI generator.
For instance, if the value is `FooApi`, the generator removes the `Api` suffix. Consequently, the Java class name will be `FooApi`.
:::
...
Only the right nesting is necessary, that is, the two fields `x-sap-cloud-sdk-operation-name` and `x-sap-cloud-sdk-api-name` should be assigned to an operation (get, post, ...).
:::
