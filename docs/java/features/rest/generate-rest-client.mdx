...
Maven will run the generator within the `generate-sources` phase which is executed before compile. Note the phase "generate-sources" is just a recommendation from our 
