...
After you created an SDK-based Java app, you can invoke the SAP BTP Business Rules REST API in your business logic.
...
The `ScpCfServiceDestinationLoader.getDestinationForService` API currently only works out of the box for a handfull of services.
...
You can check the SAP BTP Business Rules API's model definition on the [SAP API Business Hub](https://api.sap.com/package/SAPCPBusinessRulesAPIs?section=Artifacts) or simply use your IDE to discover available properties via its auto-complete function.
...
You can configure the destination to point to the Rule Execution API by simply adapting the Name, the URL, and the Token Service URL fields accordingly.
:::
...
That's it, the invocation to the java client library would remain the [same](btp-business-rules-rest-api#invoke-the-apis) and no more further changes are required.