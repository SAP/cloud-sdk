...
## Type-safe client for RESTful services in Java

The SAP Cloud SDK for Java is prepared to generate type-safe clients using [Open API](https://www.openapis.org/) to conveniently work with various REST APIs available at [SAP API Hub](https://api.sap.com/).
...
Depending on the scope modules could be available only internally within SAP or publicly released. The [SAP Cloud Platform Workflow API](clients/scp-workflow-rest-api) is an example of a publicly released API module.
...
- You're getting best in class support for your application or extensions directly from the Cloud SDK development team
...
## Do you plan to release an Open API generator? Like you've done for OData?

Because of the less deterministic standard and the huge variety of Open API services we decided not to release it as of yet.
We might reconsider this decision if we see a strong use-case and great value for our customers behind such commitment.
The value should outweigh an effort to provide and support such a feature in general public availability.
