...
SAP Cloud SDK for Java is prepared to generate a type-safe clients using [Open API](https://www.openapis.org/) to conveniently work with various REST APIs available at [SAP API Hub](https://api.sap.com/).
...
### Pre-generated type-safe REST API client
...
Depending on the scope modules could be available only internally within SAP or publicly released. [SAP Cloud Platform Workflow API](clients/scp-workflow-rest-api) is an example of publicly released API module.
...
- You'll benefit from type-safe client when accessing a service of your choice
...
Because of less deterministic standard and huge variety of Open API services we decided not to release it as of yet.
