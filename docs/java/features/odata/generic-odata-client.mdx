...
Let's look how you can work around that with generic OData client for SAP S/4HANA `YY1_QUANTITYCONTRACTTRACKI_CDS`service

In the code snippet below we:

1. Initialize generic OData Batch request with service path and OData protocol version.
1. Create a Map establishing relation between Contract object and Read request.
1. Build Read requests for contracts we want to fetch and add them to Batch request and store in Map of Read requests.
