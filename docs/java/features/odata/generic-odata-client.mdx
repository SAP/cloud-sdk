...
```java
new ODataRequestRead(servicePath, structuredQuery);
// e.g.
new ODataRequestRead("/path/to/service", structuredQuery);
```

:::tip
Structured queries can be created conveniently as described in [Building Queries below](#building-queries).
:::
