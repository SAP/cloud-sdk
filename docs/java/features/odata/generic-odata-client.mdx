...
When we release a support for type-safe calls you can simple replace this snippets with the fluent API.

### Adding Trailing Slash to the Service Path

By default SDK strips all trailing slashes from a resource path.
It's a standard behavior and works for 99% of productively available services.
Sometimes services might complain or even fail if services path doesn't end with a slash `/`.

- `https://host/servise-path/resource-path/?$filter...` - works
- `https://host/servise-path/resource-path?$filter..` - doesn't work (default URI composition strategy by SDK)

To workaround this situation you can use generic OData client
