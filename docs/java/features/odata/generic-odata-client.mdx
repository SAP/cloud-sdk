...
Instead use the action class if the operation should use POST.
:::
...
Let's look at how you can work around that with a generic OData client for SAP S/4HANA `YY1_QUANTITYCONTRACTTRACKI_CDS`service

In the code snippet below we:

1. Initialize generic OData Batch request with service path and OData protocol version.
1. Create a Map establishing the relation between the Contract object and the Read request.
1. Build Read requests for contracts we want to fetch and add them to Batch request and store them in Map of Read requests.
1. Make a Batch request
1. Collate response of the Batch request to Read requests stored in the Map
...
This example is a great demonstration how you can workaround certain limitation of type-safe client aka VDM, tackle advance OData features, or even take care of misbehaving servers or OData standard deviations.
