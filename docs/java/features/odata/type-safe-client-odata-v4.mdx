...
```java
service().getAllBusinessPartner()
         .select(new BusinessPartnerField<>(new SimpleProperty.NumericInteger<>(BusinessPartner.class, "ShoeSize"),()->"AnotherCustomField")
         .filter(new SimpleProperty.String<>(BusinessPartner.class, "ShoeSize").equalTo("42"))
         .execute(destination)
```
