...
Please, [create a feature request](https://github.com/SAP/cloud-sdk/issues/new?assignees=&labels=feature+request&template=feature_request.md&title=) if you require this feature.
:::

The [OData v4 standard ](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_AsynchronousRequests) has introduced means to make asynchronous requests.
...
At the moment the SAP Cloud SDK for Java does not provide a typed API to make such requests. However, you can leverage our [Generic OData Client](generic-odata-client.mdx) to simplify the logic of making such requests.
