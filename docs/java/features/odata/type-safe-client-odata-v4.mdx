...
:::tip Obtaining the Entity to Operate on
To get the entity object to operate on it is genereally recommended to download it from the service first via its respective `get..ByKey` method.
In case of actions this will also ensure that ETags are being handled.

But you can also use the builder on the entity object to create an instance, if you are aware of the key properties that are required.
:::
...
Furthermore OData allows to invoke bound operations not only on entities but also on navigation properties.
...
Such a request can be build with the API as follows:
