...
It is represented by the `BusinessPartnerService` class which is part of the pre-generated type-safe client for SAP S/4HANA also known as Virtual Data Model (VDM).
