...
You can always [generate a type-safe OData client library](generate-odata-client.mdx) based on API specifications available on the [SAP API Business Hub](https://api.sap.com/) or found elsewhere.
...
If you need a client for an OData service where we do not ship a pregenerated client use our [code generator](generate-typed-odata-v2-and-v4-client-for-java) to convert its service definition into the type-safe Java client library.
