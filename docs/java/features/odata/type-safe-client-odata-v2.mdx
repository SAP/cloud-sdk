...
We recommend to check what filter options are supported by [OData v2 standard](https://www.odata.org/documentation/odata-version-2-0/uri-conventions/). Search for: **4.5. Filter System Query Option ($filter)** after navigation to the link.

The SDK currently has support for the following filtering functions in OData v2 client:

| Filter name           | Filter function | Example ODava v2 query string                |
|-----------------------|-----------------|----------------------------------------------|
| Equals                | `eq`            | /Suppliers?$filter=Address/City eq 'Redmond' |
| No Equals             | `ne`            | /Suppliers?$filter=Address/City ne 'London'  |
| Less than             | `lt`            | /Products?$filter=Price lt 20                |
| Greater than          | `gt`            | /Products?$filter=Price gt 20                |
| Greater than or equal | `ge`            | /Products?$filter=Price ge 10                |
| Less than or equal    | `le`            | /Products?$filter=Price le 100               |

:::note  "Header"
We are working on supporting string filtering function like `SubstringOf`, `endsWith`, `startsWith`, etc.
:::

For more details check our [API documentation](https://help.sap.com/doc/b579bf8578954412aea2b458e8452201/1.0/en-US/com/sap/cloud/sdk/datamodel/odata/helper/package-summary.html) or use `autocomplete` function of your IDE to see available methods of the provided Fluent API.
