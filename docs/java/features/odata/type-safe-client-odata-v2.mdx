...
Or the typed API doesn't support a specific feature just yet.

For such cases you can leverage the more [low-level generic OData Client](./generic-untyped-odata-client).
:::
...
OData Function Import can be also used inside of OData Batch ChangeSets, for that use the method `addFunctionImport(...)`.
