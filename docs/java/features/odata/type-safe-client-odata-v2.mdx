...
title: Typed Client To Consume OData v2 Services
hide_title: false
...
<!-- vale SAP.Headings = NO -->

## Versions of OData v2 Client

<!-- vale SAP.Headings = YES -->
...
```
If you don't know the name of the custom fields added by the customer, you can still use `getCustomFieldNames()` to obtain the set of custom fields introduced and use this set further on to pull values from the entity.
For e.g.
...
The accessors for custom fields respect multitenancy, for e.g. `getCustomFieldNames()` would only fetch custom field names available in the context of the current tenant.

### Using Custom Fields in system query options
You can also pass custom fields to select or filter system query options.

<Tabs groupId="customField" defaultValue="v2" values={[ { label: 'OData V2', value: 'v2', }, { label: 'OData V2 (Deprecated)', value: 'v2-deprecated', } ]}>
...
```java
service().getAllBusinessPartner()
         .select(new BusinessPartnerField<>("ShoeSize"),()->"AnotherCustomField")
...
Please be aware that such an example can work for all tenants only if `ShoeSize`, `AnotherCustomField` custom fields has been defined by all tenants.
...
- `executeRequest` throws the runtime exception `ODataException` and respective subclasses.
