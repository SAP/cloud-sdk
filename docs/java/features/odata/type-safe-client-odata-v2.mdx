...
It is represented by the `BusinessPartnerService` class which is part of the pre-generated SAP S/4HANA Virtual Data Model (VDM). The following code snippets assume that an instance of this service is set up:
...
For create, update and delete requests the SAP Cloud SDK will try to send a [CSRF token](https://en.wikipedia.org/wiki/Cross-site_request_forgery#Cookie-to-header_token).
