...
sidebar_label: Generate a Typed OData Client
...
:::caution
In most cases, you will have to override the generated service path in the code.
...
In case you run into issues with the above process: Double check your service and file names, check that the folders are set up correctly and that the service name mappings meet your expectations.
...
| `<nameSource>`                 | `LABEL`  | Source for entity and property names (either `LABEL` or `NAME`). **Caution:** Works only in conjunction with the default `<namingStrategy>` |
...
| `--use-odata-names`                |  none  |    -     | Generates entity and property names based on the names within the EDMX file instead of the `sap:label`. **Caution:** Works only in conjunction with the default `--name-strategy-class` |
...
| `withNameSource(NameSource)`               |    -     | Source for entity and property names (either `DefaultNamingStrategy.NameSource.LABEL` or `DefaultNamingStrategy.NameSource.NAME`). **Caution:** Works only in conjunction with the `DefaultNamingStrategy` |
