...
The SAP Cloud SDK also offers the `TimeScopedHttpClientCache`, which keeps existing `HttpClient`s for a certain period of time.

Switching the default cache can be done as follows:
