...
:::danger
Currently support for Certificate based Authentication from SAP Cloud SDK is limited to `XSUAA` service.
Support exists but is not tested extensively yet for `Destination` and `Connectivity` services.
:::
...
To obtain an access token or `JWT`, first we extract the `clientid`, `clientsecret` and `uri` from the service binding and then using the `url` from `xsuaa` service binding, trigger a [`token flow`](https://github.com/SAP/cloud-security-xsuaa-integration/tree/main/token-client) to obtain a `JWT`.
...
:::info Hint
Certificate based authentication is to be the default authentication mechanism on SAP BTP Cloud Foundry starting from Sept. 13th 2021.
:::
