...
To obtain an access token or `JWT`, first we extract the `clientid`, `clientsecret` and `uri` from the environment variable of the service binding and then using the `url` from `xsuaa` service binding, trigger a [`Client credentials token flow`](https://github.com/SAP/cloud-security-xsuaa-integration/tree/main/token-client) to obtain a `JWT`.
...
This means that in the environment variable of the service binding in addition to the `client_secret` you will also find a `certificate` entry now.
For e.g.
```
{
    "VCAP_SERVICES": {
