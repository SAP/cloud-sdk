...
## Usage of `client_secret` and shortcomings
...
Please note that the process of obtaining the `JWT` was internally carried out by the SAP Cloud SDK and developers need not worry about doing this explicitly.
...
This means that in the environment variable of the service binding in addition to the `client_secret` you will also find a `certificate`now.
For e.g.
```
{
    "VCAP_SERVICES": {
...
```
This certificate would now be used to establish a secure session to obtain the `JWT` instead of the `clientsecret` used previously.
Please note that the process of obtaining the `JWT` is still internally carried out by the SAP Cloud SDK, we just use the `certificate` now if the binding contains the `certificate`.
