...
1. SAP BTP Connectivity service
...
<details>
    <summary>Details and example values of a destination retrieval steps</summary>
...
:::tip (Beta) Authorization Token Forwarding
...
1. Lookup via SAP BTP Destination service
   ...
   The SAP Cloud SDK tries to fetch the destination from the SAP BTP Destination service the destination name.

1. Lookup via SAP BTP Connectivity Service
...
### Configuration Steps for Local Development Environment

You can either try CAP Development Kit for automated setup or configure everything manually.

### Automatically inject service bindings via CAP Development Kit
...
To achieve that you have to bind them and inject via [CAP Development Kit](#automatically-inject-service-bindings-via-cap-development-kit) or by manually providing your [`VCAP_SERVIES`](#create-and-provide-vcap_services-variable) variable.
