...
Using the "URL.headers." prefix for the key argument makes the key-value pair considered as a header.
Only the remaining part of the key will be used to create the header.
...
The method `.property()` can be called on the destination.
Using the "URL.queries." prefix for the key argument makes the key-value pair considered as query parameters.
Only the remaining part of the key will be used in the query string.
