...
We provide support for the following authentication types:
...
For each registered implementation it invokes the method `getHeaders(destination)` and appends the returned headers to the currently prepared request.
