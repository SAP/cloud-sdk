...
To not loose the session in between requests, the SAP Cloud SDK offers the `TimeScopedHttpClientCache` (as opposed to the `RequestScopedHttpClientCache`, which is used by default).
Switching the cache can be down as follows:
