...
Besides the retreival strategy, the SAP Cloud SDK also offers a _token exchange strategy_ that lets you change the behavior when authentication requires a user token exchange.
...
This is needed to get the authentication type configured in the destination, which might then require a user token exchange.

In special cases, for example, if you know that the destination you are about to retrieve requires a user token exchange, performance can be improved by skipping the initial "look up" request like so:
