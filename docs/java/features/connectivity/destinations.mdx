...
We have to perform some configuration steps to tell the SAP Cloud SDK how to access the SAP BTP destination service and connectivity service.
...
If you are developing your application in Business Application Studio (BAS), reaching On-Premise systems can be achieved by performing the following steps:

1. Make sure your BAS instance is part of the same CF space as your Cloud Connector.
1. Add two properties called `WebIDEEnabled` and `HTML5.DynamicDestination`, both with value `true`, to your On-Premise destination.
...
Please refer to [the BAS connectivity guide](../../../js/guides/bas-external-system) created for the JavaScript SDK to get more information and a detailed description of the technical background. The information there equally applies to the Java SDK.
