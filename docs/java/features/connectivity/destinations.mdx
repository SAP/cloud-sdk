...
Besides the retreival strategy, the SAP Cloud SDK also offers a _token exchange strategy_ that lets you change the behavior when authentication requires an user token exchange.
...
By default, the SAP Cloud SDK first performs a "look up" request against the destination service, which yields the destination.
This is needed to get the authentication type configured in the destination, which might then require an user token exchange.

In special cases, for example, if you know that the destination you are about to retrieve requires an user token exchange, performance can be improved by skipping the initial "look up" request like so:
