...
:::caution Destinations are cached for 5 minutes

Mind that Destinations are cached for 5 minutes.
...
We plan to make this cache configurable.
...
:::tip "Use CAP Development Kit for local service bindings"
...
Now, by using `cds bind` command you can start any Java app with service binding injected into your local environment.
It will create a `VCAP_SERVICES` variable containing service keys from a Cloud Foundry space or Kubernetes cluster without storing credentials on a disk.
Start your Java application through the `cds bind --exec command` to use the service bindings.
For example:
...
Find more details in the [official CAP documentation](https://cap.cloud.sap/docs/advanced/hybrid-testing#run-with-service-bindings).
:::
...
So they will always be included in the query string when calling the target system.
...
:::tip (Beta) AuthToken Forwarding
Sometimes you might want to forward the value of the `Authorization` header on the current request directly to the target system.
For that you can add the property `"forwardAuthToken": "true"` to your destination.
This will forward the header, without any additional changes, to the specified target system.
:::
...
To access the **Destination** and **Connectivity** service on SAP BTP we have to configure the SAP Cloud SDK accordingly.
...
##### Developing in Business Application Studio
...
Please refer to [the BAS connectivity guide](../../../js/guides/bas-external-system) created for the SAP Cloud SDK for JavaScript to get more information and a detailed description of the technical background.
