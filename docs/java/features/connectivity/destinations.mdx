...
Using this default strategy results into deleting the `HttpClient` (together with the session cookie) that was used to auhtorize at the target system, once the initial request has been handled.
