...
Previously (prior to version `3.61.0`), the implementations were relying on Java's `Servlet` API.
...
As mentioned in the beginning, prior to version `3.61.0`, the SAP Cloud SDK's conveniene _accessors_, such as the `AuthTokenAccessor`, used to rely on the incoming `ServletRequest`.
...
That way, you can add, remove, and overwrite existing headers easily:
...
### Forwarding Headers to other Cloud SDK Features

Most of the Cloud SDK's functionality that relies on reading HTTP headers has been migrated to use the new `RequestHeaderAccessor` - and therefore also the `RequestHeaderContainer`.
Therefore, explicitly defining which headers should be used by the Cloud SDK is essential in many scenarios.
