...
SAP Cloud SDK does _not_ package JCo as a library with its dependencies. The SDK assumes that JCo is available on the JVM classpath.
:::
...
To make that compile, we add Lombok to our `application/pom.xml` as dependency.
