...
SAP Cloud SDK offers a convenient Java API on top of JCo that integrates with other SAP Cloud SDK core concepts, such as the destination retrieval on SCP, as well as a nice way to mapping Java entity classes to BAPI input parameters and result sets.
...
On the BTP Neo landscape, JCo is automatically provided by the infrastructure, on the BTP Cloud Foundry environment landscape one must use the SAP Java Build Pack during app deployment.
...
We build an app that we deploy to the BTP Cloud Foundry environment.
...
- _destinationName_: Defines which BTP destination to use for connecting to SAP S/4HANA.
- _controllingArea_: Defines the controlling area for the cost center retrieval.

The app retrieves the BTP destination with SAP Cloud SDK core capabilities, creates a BAPI request, and issues that to SAP S/4HANA through the SAP Cloud Connector.
...
Thereafter we instruct the SAP Cloud SDK to retrieve the BTP destination per the provided name.
...
On the return type of `result.get("COSTCENTERLIST")` you call `getAsCollection()` to treat this parameter as a collection and then you instruct the SAP Cloud SDK to transfer the contained entries into individual `CostCenter` instances with `asList(...)`.
...
### Deploy to SAP Business Technology Platform - Cloud Foundry environment

Invoke `cf push` from the directory root to deploy the app to your SAP Cloud Foundry environment space.

### Configure SAP Business Technology Platform - Cloud Foundry environment
...
- BTP Cloud Foundry environment subaccount with entitlements for XSUAA, destination and connectivity service
- SAP Cloud Connector connected to the SAP Cloud Foundry environment subaccount (if SAP S/4HANA is On-premise)
...
Furthermore, if you want to call JCo from any newly created thread, please check out the documentation [here](https://help.sap.com/viewer/cca91383641e40ffbe03bdc78f00f681/Cloud/en-US/b14eeb94be944c62a5880bde50c59aef.html).
Be aware to exclude the `java-api` artifact, since this is transitively included with the SAP Cloud SDK!
...
Open the BTP cockpit on the subaccount level.
...
- On BTP Cloud Foundry environment, the SAP Java Build Pack brings JCo as a dependency.
...
At the top of your dependency management section, before including any SAP Cloud SDK dependencies in your project, insert the following:
