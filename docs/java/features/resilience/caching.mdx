...
If you need fine-grained influence on which cache entries to remove, you may write your custom `CacheFilter`.


```java
// CacheFilter which matches all cache entries belonging to tenant "tenant1"
CacheFilter tenantFilter = (( configuration, cacheKey, cacheEntry ) -> cacheKey.getTenantId().get().equals("tenant1"));
...
You can even utilize multiple `CacheFilter` instances for the cache clearing:
...
// clears all cache entries which match the provided CacheFilter instanes
ResilienceDecorator.clearCache(resilienceConfig, tenantFilter, principalFilter);
```
All provided cache filters are invoked in the given order.
