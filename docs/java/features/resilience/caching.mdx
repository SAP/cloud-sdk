...
This clears cache entries associated with the resilience configuration, the current tenant, and the principal depending on the configured isolation mode.
Currently, however, additional parameters that may be specified within the `CacheConfiguration` are not considered.

If you need to manually clear the entire cache associated with a resilience configuration, you may do so by:

```java
ResilienceDecorator.forceClearCache(resilienceConfig);
```
...
where `SimpleCacheEntryListener` is a class that implements the interfaces of the required events with respect to cache defined in [EventType](https://javadoc.io/doc/javax.cache/cache-api/latest/javax/cache/event/EventType.html).
