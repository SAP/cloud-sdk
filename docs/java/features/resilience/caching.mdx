...
The SDK provides prepared cache filters you can reuse for matching on tenant, principal or parameters.

```java
CacheFilter matchesCurrentParameters = CacheFilter.keyMatchesParameters();

CacheFitler matchesCurrentTenant = CacheFilter.keyMatchesTenant();

CacheFitler matchesCurrentPrincipal = CacheFilter.keyMatchesPrincipal();
```
...
CacheFitler matchesMytenant = CacheFilter.keyMatchesTenant(myTenant);

Principal myPrincipal = obtainPrincipal();
CacheFitler matchesMyPrincipal = CacheFilter.keyMatchesPrincipal(myPrincipal);
```
