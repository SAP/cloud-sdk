...
:::info
The term _Tenant_ is a concept used by the XSUAA Service on SAP BTP. In that context a Tenant has a 1:1 relationship to the subaccount a multitenant application is subscribed in.
An extension of this is the _Zone_ concept, used by the IAS Service on SAP BTP. Here you can group multiple subaccounts into a single zone.
On side of the consuming application this doesn't make a difference.
