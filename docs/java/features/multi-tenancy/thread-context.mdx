...
In case you are using CAP (with the `cds-integration-cloud-sdk` dependency) the Tenant & Principal information will **instead** be derived from the [CAP Request Context](https://cap.cloud.sap/docs/java/request-contexts).
That also means that the `ThreadContext` will not be used when accessing this information.
All other information (not Tenant or Principal) will still be derived & maintained via the `ThreadContext`.
:::
...
When using CAP the Tenant & Principal information is derived from the `RequestContext`.
...
:::caution CAP Integration
In case you are using CAP also the CAP Request Context needs to be passed on.
Please refer to [this guide](../../guides/cap-sdk-integraion) on how to achieve this.
:::
