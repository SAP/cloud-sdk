...
```java
ExecutorService executor = ExecutorDecorator.newDefaultExecutor();
executor.execute(
    // code that is executed within the ThreadContext propagated by ExecutorDecorator
    () -> operation());
```

<details>
<summary> Customizing ThreadContext </summary>

It's possible to customize the `ThreadContext`:

```java
ThreadContext context = new DefaultThreadContext();
context.setPropertyIfAbsent("name", "value");

ThreadContextExecutor executor = new ThreadContextExecutor().withThreadContext(context);
Callable operationWithContext = () -> executor.execute(() -> operation());

invokeAsynchronously(operationWithContext);
```

Take note that the `ThreadContextExecutor` is created _before_ executing the asynchronous operation.
...
Annotating a method with `@Async` will make it execute in a separate thread.

To enable this, a configuration class has to be created:
...
public class AsynchronousConfiguration implements AsyncConfigurer {
  ...
  public Executor getAsyncExecutor() {
    return ExecutorDecorator.decorate(new SimpleAsyncTaskExecutor());
  }
}

```

For your annotated methods to be able to access the `ThreadContext`, an `ExecutorDecorator` needs to be supplied.
