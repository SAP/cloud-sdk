...
This will use a default `ThreadContextExecutorService` based on a `CachedThreadPool` to create a new Thread and run the task.
...
In case you are using CAP, note that the CAP Request Context also needs to be passed on (in addition to the `ThreadContext`).
...
public class AsynchronousConfiguration implements AsyncConfigurer {
  ...
  public Executor getAsyncExecutor() {
    return ThreadContextExecutors.getDefault();
  }
}
```
...
The `ThreadContextExecutors` class leverages a default `ExecutorService` that can be configured.
...
// You can also set this executor as the default for ThreadContextExecutors:
ThreadContextExecutors.setDefault(executor);
...
```java
Tenant myTenant = new DefaultTenant("foo");
ThreadContextExecutor customExecutor = new ThreadContextExecutor()
            .withListeners(new TenantThreadContextListener(myTenant));
...
In such cases, a new instance of `RequestHeaderContainer` can easily be created in a few different ways.
...
Therefore, updating an already existing instance is not possible.
...
Last but not least, your application should always serve German customers and, therefore, you need to make sure the `Accept-Language` header is always adjusted accordingly.

Luckily, using the `Builder` API, fulfilling these requirements is straightforward:
