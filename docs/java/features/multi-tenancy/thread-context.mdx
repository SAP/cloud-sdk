...
:::info
The term _Tenant_ is a concept used by the XSUAA Service on SAP BTP to distinguish between different subaccounts of a multi-tenant application. 
A _Tenant_ maps 1:1 to the multi-tenant application's subaccount where it is subscribed.

An extension of the _Tenant_ concept is the _Zone_ concept introduced by the IAS Service on SAP BTP.
It allows you to group multiple subaccounts into a single zone.

For development purposes, there's currently no difference between _Zone_ and _Tenant_.
That is why the SAP Cloud SDK for Java decided to use the term _Tenant_ to refer to both the XSUAA-based subaccount _Tenant_ and IAS-based _Zone_.
:::
