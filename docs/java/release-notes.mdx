...
- The module `auditlog-scp-cf` is considered **deprecated**. 
  The internally used auditlog API is outdated and no longer adheres to product standard.
  In the future Cloud SDK may provide an update leveraging the recommended API, until then the users have the option to continue using the deprecated logic at their own risk, or directly consume the recommended auditlog API directly - without the Cloud SDK convenience layer.
  This deprecated module will remain in upcoming Cloud SDK v4.
...
- Fix incorrect encoding of Pipes (`|`) in OData queries
- Prevent compilation errors on nullable properties when using the OpenAPI generator
