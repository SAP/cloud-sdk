...
- Fix an issue where custom service-path information are not propagated from OData V2 service classes to batch classes.
  Previously this let to a runtime exception: `ODataRequestException: Batch request contains requests to different service paths [...]`.
...
- The OpenAPI generator now fails with a helpful message during generation if the input [](<[specification](https://swagger.io/docs/specification/data-models/oneof-anyof-allof-not/)>) contains any of these keywords `oneOf` or `anyOf` in either:
