...
- We introduced the API `ResilienceDecorator.clearCache(ResilienceConfiguration, CacheFilter)` which allows to pass a `CacheFilter` instance.
This allows fine-grained influence on which cache entries shall be cleared from the cache.
