...
[Maven Central](https://search.maven.org/search?q=g:com.sap.cloud.sdk*%20AND%20v:3.54.0) | [Javadoc](https://help.sap.com/doc/55bb1642ea614bf582d514ed34401136/1.0/en-US/index.html)

### Known Issues

- We are aware of the vulnerability [CVE-2021-37137](https://www.whitesourcesoftware.com/vulnerability-database/CVE-2021-37137) that affects [Netty](https://netty.io/news/2021/08/16/4-1-67-Final.html) in version `4.1.67.Final`. Unfortunately, however, we are not yet able to resolve that vulnerability due to [an issue](https://github.com/netty/netty/issues/11691) with the `4.1.68.Final` release of Netty, which prevents projects from compiling. We will update the related dependencies once the build issue has been resolved. In the meanwhile, consumers of the SAP Cloud SDK do not need to worry about the vulnerability as it affects only internal and deprecated modules that are not recommended to use.
...
- Applications that use an **IAS** binding instead of XSUAA are now supported out of the box.
  ...
  - IAS tokens can be used for Principal Propagation and User Token Exchange
  - Under the hood the SDK now uses zone IDs to pass tenant information to IAS (and XSUAA, see also the note under "improvements")
