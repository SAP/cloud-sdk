...
- We enabled support for certificate-based authentication for services on Cloud Foundry which allows services to use Mutual Transport Layer Security or mTLS instead of providing client credentials. In the future, this will become the default authentication method on the BTP Cloud Foundry.
  ...
  - Please note: you might see `WARN` messages in the logs. It's produced by the Java Security Library that we use as a dependency. We verified the logic with the library publisher and it's safe to use. The false log message is soon to be removed.
    However, the logic is safe to use, the message will be removed in the near future.

- We enabled the usage of certificate without password protection for the Destination Service on Cloud Foundry.

- We updated the remaining OData VDM services to the [newest release 2108 of SAP S/4HANA Cloud](https://news.sap.com/2021/08/sap-s4hana-cloud-2108-engineering-innovations/). Operations and datatypes of the following services may have faced backwards-incompatible changes, that can be explored in the [SAP API Business Hub for SAP S/4HANA Cloud](https://api.sap.com/products/SAPS4HANACloud/overview):
