...
[Maven Central](https://search.maven.org/search?q=g:com.sap.cloud.sdk*%20AND%20v:3.42.0) | [Javadoc](https://help.sap.com/doc/55bb1642ea614bf582d514ed34401136/1.0/en-US/index.html)

### New Functionality
- We improved OData V4 update requests with PATCH by allowing for adding complex properties without the need for using setters for each value inside the complex property. 
  For the property you can use the builder pattern or the constructor to propagate the values.
  Before:
  ```java
  Entity entity;
...
- The OData V2 generator is able to maintain argument order in function imports and get-by-key fluent helpers over successive generations, even when the originating specification changes parameter order.
  This implicit behavior can be disabled by adding the flag to the plugin configuration: `<keepExistingSignatures>false</keepExistingSignatures>`
- We updated the OData v2 VDM for the remaining services:
  ...
  Now the SAP Cloud SDK again supports the latest state of all OData services listed in the [SAP API Business Hub for SAP S/4HANA Cloud](https://api.sap.com/package/SAPS4HANACloud?section=Artifacts).
