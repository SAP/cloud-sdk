...
Version 2.x of the SAP Cloud SDK for Java V2 has reached its end of life and is no longer maintained. To continue using the latest features outlined in the release notes for version 3 below, please [upgrade to **SDK V3**](https://developers.sap.com/tutorials/s4sdk-migration-v3.html).
...
- The VDM classes for `TimeDependentStockLevelService` have been removed as the SAP S/4HANA on-premise OData service does not exist in releases 2020 FPS0 or FPS1.
- With SAP S/4 OnPremise release 2020 FPS1, in the VDM class for `VehicleManagementSystemForVehicleService` the `getVMSVehicleIncomingInvoiceByKey` method now takes only two parameters ( `vMSVehicleUUID` and `vMSVehicleActionDateTime`) as input as opposed to three before ( `vMSVehicleUUID`,`vMSVehicleActionDateTime` and `vMSVehicleActionDocType` ).

### New Functionality

- The [type-safe OData client](https://sap.github.io/cloud-sdk/docs/java/features/odata/overview#type-safe-odata-client) (VDM) is updated to the latest release (FPS1) of SAP S/4HANA on-premise 2020.
...
- Enable the OData V2 and V4 VDM generators to generate service methods for multiple entity sets belonging to the same entity type.
  Formerly only the first entity set of an entity type was regarded.
  By default, to ensure backwards compatibility this feature is turned off.
...
- The OpenAPI Generator now allows for setting a custom copyright header.
  Use the option `<copyrightHeader>` to set a custom header, or set the flag `<sapCopyrightHeader>` to have a SAP copyright header added.
  By default, no header is generated.

### Improvements

- The OData V2 VDM classes and generator now support `null` as parameter values for function import methods when applicable.
...
- Fixed an issue where selecting complex properties in the OData V4 VDM would lead to an incorrect HTTP query.
- Fixed an issue where very long RFC function names (>= 30 characters) would lead to an incorrect `SOAPAction` header.
...
- Fix an issue where operations with empty payload in OData Batch request changesets on S/4HANA OData services failed with _Bad Request_ responses.
...
- Renamed/Moved Modules. Our goals with the renaming were the following:
  ...
  - The term `s4hana` in the group id is only used if the contained module is in some way specific for S/4HANA.
