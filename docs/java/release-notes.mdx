...
[Maven Central](https://search.maven.org/search?q=g:com.sap.cloud.sdk*%20AND%20v:3.41.0) | [Javadoc](https://help.sap.com/doc/55bb1642ea614bf582d514ed34401136/1.0/en-US/index.html)
...
- Dependency Updates:
  - SAP dependency updates:
    - Update BTP Neo Java Tomcat API from `3.123.6.2` to `3.124.9.2`
    - Update BTP Neo Java TomEE7 API from `1.92.6.2` to `1.93.10.2`
...
- Dependency Updates:
  - SAP dependency updates:
    - Update BTP Neo Java Tomcat API from `3.121.8.1` to `3.123.6.2`
    - Update BTP Neo Java TomEE7 API from `1.90.9.1` to `1.92.6.2`
...
- With SAP S/4 On-premise release 2020 FPS1, in the type-safe client (VDM) class for `VehicleManagementSystemForVehicleService` the `getVMSVehicleIncomingInvoiceByKey` method now takes only two parameters ( `vMSVehicleUUID` and `vMSVehicleActionDateTime`) as input as opposed to three before ( `vMSVehicleUUID`,`vMSVehicleActionDateTime` and `vMSVehicleActionDocType` ).
...
- We now support the `SAMLAssertion` authentication type on Cloud Foundry environment.
...
- We introduced a new API to build a `Destination` for connecting to services on Cloud Foundry environment.
...
- For BTP Cloud Foundry environment, instances of `DestinationHeaderProvider` will be called for every HTTP request as of now. Earlier versions invoked the API only once during destination construction.
...
- Dependency Updates:
  - SAP dependency updates:
    - Update BTP CF Java Logging Support from `3.2.1` to `3.3.0`
    - Update BTP Neo Java Tomcat API from `3.119.9` to `3.121.8.1`
    - Update BTP Neo Java TomEE7 API from `1.88.6` to `1.90.9.1`
...
- Fix an issue where dependency conflicts between the SDK and the SAP Java Buildback lead to runtime issues on the BTP Cloud Foundry environment.
...
- The BTP Workflow API client library in module [`scp-workflow-cf`](https://search.maven.org/artifact/com.sap.cloud.sdk.services/scp-workflow-cf) is now generally available and released for productive usage.
...
- Dependency Updates:
  ...
  - Support for Logback for BTP CF from `3.0.1` to `3.2.0`
...
- The BTP service "Hyperledger Fabric on SAP Business Technology Platform" is deprecated, the corresponding Maven dependency `com.sap.cloud.sdk.services:scp-blockchain` gets deprecated too. Use the dependency [`com.sap.cloud.sdk.serices:blockchain-client-fabric`](https://mvnrepository.com/artifact/com.sap.cloud.sdk.services/blockchain-client-fabric) instead.
...
- Released new pre-generated type-safe client libraries for the OData v4 services in the [latest RTC release 2020 of SAP S/4HANA On-premise](https://blogs.sap.com/2020/10/07/sap-s-4hana-2020-release/)
...
- We released new pre-generated type-safe client libraries (also known as the Virtual Data Model - VDM) for the [latest RTC release 2020 of SAP S/4HANA On-premise](https://blogs.sap.com/2020/10/07/sap-s-4hana-2020-release/). Except for the following services, everything else available in [SAP API Business Hub for SAP S/4HANA On-premise 2020](https://api.sap.com/package/S4HANAOPAPI?section=Artifacts) are available:
...
- The experimental REST BTP Workflow client library got enhanced with a new API class constructor that expects an `ApiClient`, which allows for easier integration with e.g., the Spring `RestTemplate`.
...
- We added support for new authentication types recently introduced on SAP Business Technology Platform - Cloud Foundry environment. It further improves handling of `Destinations` and keeps SDK up to date with BTP features:
  - `SAP_ASSERTION_SSO`
  - `OAuth2JWTBearer`
  - `OAuth2Password`
...
- Fix an issue on BTP Neo with anonymous HTTP requests being blocked, when optional module `concurrency-scp-neo` was added as application dependency.
...
- Method and parameter names for the BTP Workflow client library were adjusted
...
- Client Library for BTP Workflow:
...
- Add `getAllDestinations(DestinationOptions options)` in `ScpCfDestinationLoader` class to fetch all the Cloud Foundry environment destinations. The method prioritizes the destination at service instance level in case there exists a destination with the same name on service instance and on sub account level.
...
- Introducing the client library for the Workflow API on SAP Business Technology Platform, Cloud Foundry environment
...
- Introduce the module _SCP Blockchain Client Fabric_ for easy interaction with a Hyperledger Fabric network using all necessary configuration and credentials from a standardized user-provided service in the Cloud Foundry environment environment.
...
- Enable the [`PrincipalAccessor`](https://help.sap.com/doc/4cc23f4c3bf147fe834fbd0e3413e5d5/1.0/en-US/index.html?com/sap/cloud/sdk/cloudplatform/security/principal/PrincipalAccessor.html) to also read the current principal from the newly introduced [`BasicAuthenticationAccessor`](https://help.sap.com/doc/4cc23f4c3bf147fe834fbd0e3413e5d5/1.0/en-US/index.html?com/sap/cloud/sdk/cloudplatform/security/BasicAuthenticationAccessor.html), in addition to the already existing way to read it from the current JWT. This functionality applies to Cloud Foundry environment. A valid JWT takes precedence over a Basic Authentication header.
...
- The connectivity to on-premise systems from BTP Cloud Foundry environment is updated, so that the [authentication](https://help.sap.com/viewer/cca91383641e40ffbe03bdc78f00f681/Cloud/en-US/39f538ad62e144c58c056ebc34bb6890.html) now follows the [recommended](https://help.sap.com/viewer/cca91383641e40ffbe03bdc78f00f681/Cloud/en-US/39f538ad62e144c58c056ebc34bb6890.html) protocol for principal propagation.
...
- Support reading the principal from an OAuth SAML Bearer token on Cloud Foundry environment.
...
- Fix an issue concerning HTTP destinations on Cloud Foundry environment where the default JDK trust store was mistakenly taken into account even if an own trust store is defined.
...
- Verify that the Maven `artifactId` does not contain an underscore character when creating a project using the `scp-cf-spring` and `scp-cf-tomee` archetypes. This is necessary as the artifact ID will also be used as Cloud Foundry environment application name and the deployment to Cloud Foundry environment will fail if the application name contains an underscore.
...
- Removed the destination retrieval strategy configuration from the DestinationAccessor. It is no longer a global configuration by destination name.
  ...
  - e.g. on Cloud Foundry environment:
    ```
...
- Make XsuaaService public for communicating with a given XSUAA service on SAP Business Technology Platform, Cloud Foundry environment.
