...
- Currently, the TomEE based archetype for Cloud Foundry does not work with the latest SAP Java Buildpack when using Java 11.
  Please use the buildpack version `sap_java_buildpack_1_29` for this use case.
...
- Replace the deprecated constant `org.apache.commons.codec.Charsets.UTF_8` in the HelloWorldControllerTest class of the Spring archetype by its recommended successor `java.nio.charset.StandardCharsets.UTF_8`.
...
- Fix an issue where OData Batch response API did not conveniently throw or wrap an exception for failing HTTP response code health checks.
  Affected methods:
