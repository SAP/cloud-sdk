...
You can combine multiple cache filters to a disjunction by calling `CacheFilter.or(firstFilter, secondFilter)` or to a conjunction by calling `CacheFilter.and(firstFilter, secondFilter)`.  
- The OData V4 generator now supports the generation of actions with non-primitive value types, i.e. entity, complex, enum and collection value types. 
