...
- The SAP Cloud SDK now supports a new `ProxyType` named `PrivateLink`.
  This type will be present on destinations created for the [Private Link Service](https://help.sap.com/viewer/product/PRIVATE_LINK/CLOUD/en-US).
- The SAP Cloud SDK now provides a new accessor for _HTTP headers_ of the _incoming request_.
  The new `RequestHeaderAccessor` (located in the `cloudplatform-core` module) can be used to conveniently access the HTTP headers of the currently processed incoming request.
