...
- Introduce separate enums by platform for destination retrieval strategy.
...
- Configure caching within the ResilienceConfiguration class by providing an instance of CacheConfiguration class.
  - As a parameter, CacheConfiguration requires at least a value for duration until the cache entry will be expired.
...
- RequestContext is now ThreadContext
  - New concept of ThreadContextDecorator to allow decorating the callable which is wrapped in a ThreadContext.
    Difference to ThreadContextListener is that the listener is responsible for setting ThreadContext properties, while the decorator allows to decorate the callable that is wrapped in the ThreadContext.
...
- Principal replaces User.
  ...
  - No hasAuthorizations method, use authorizations with "contains" method of set
  - CacheKey and CacheManager have been adjusted accordingly to reflecting naming: user is now principal
...
- LocalScopePrefixProvider now uses Try instead of throwing exceptions which were previously not declared on the API.
...
- Add new module security-servlet containing servlet-based security filters.
...
- Decouple the implementation of resilient behavior from a concrete framework with the generic functional-style adapters in com.sap.cloud.sdk.cloudplatform.resilience. Provide a default implementation of resilience using resilience4j as underlying library.
  - Provide ResilienceDecorator as the entry point for wrapping any code (callabe or supplier) with resilient behavior.
