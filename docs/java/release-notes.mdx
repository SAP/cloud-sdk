...
- Using `null` in OData v2 filter expressions does no longer lead to a `NullPointerException` upon executing the request.
- Fix an edge case where `BigDecimal` values where not serialised correctly in OData V2 requests.
