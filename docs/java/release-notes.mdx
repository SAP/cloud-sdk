...
- The OpenAPI generator now fails with a helpful message during generation if the input []([specification](https://swagger.io/docs/specification/data-models/oneof-anyof-allof-not/)) contains any of these keywords `oneOf` or `anyOf` in either:
  ...
  This change is not considered a regression since otherwise the generated code wouldn't have been able to compile anyhow.
  Please remove these properties from your specification file for the generation to succeed.
...
- The naming of the default `ScpCfDestinationRetrievalStrategy` (`ALWAYS_SUBSCRIBER`) in the `ScpCfDestinationLoader` did not behave as the name implied, as it was taking the current tenant, which might, due to misconfiguration, also be the Provider Tenant.
  This is now fixed by the following steps:
  - Deprecation of the unclear `ALWAYS_SUBSCRIBER` strategy
  - As a default providing a renamed alternative keeping the same behavior: `CURRENT_TENANT`
