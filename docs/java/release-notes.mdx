...
## 3.55.0 - October 7, 2021

[Maven Central](https://search.maven.org/search?q=g:com.sap.cloud.sdk*%20AND%20v:3.55.0) | [Javadoc](https://help.sap.com/doc/0d9c3f5f8d874f99a32444910d7a3504/1.0/en-US/index.html)
...
- We introduced a new abstraction to load service bindings from various locations in an environment agnostic manner:
  ...
  - The `EnvironmentVariableServiceBindingLoader` that is capable of parsing environment variables (such as the `VCAP_SERVICES`) on SAP BTP Cloud Foundry
  - The `FileSystemServiceBindingLoader` that is capable of parsing service bindings, which are mounted to the file system (for example, when running in a K8s environment).
  - The `ServiceBindingMerger` that is capable of combining the results of an arbitrary amount of other `ServiceBindingLoader` instances based on a configurable service binding property (e.g. the `name` of the bindings).

  During the design of this new feature, we paid special attention to the configurability and extensibility of the architecture to enable consumers to support their own runtime with as little changes as possible.

- We changed the way our `ScpCfCloudPlatform` is loading service bindings in accordance to the newly introduced `ServiceBindingLoader` API (see note above).
  In particular, K8s based environments where service bindings are mounted to the file system are now support out-of-the-box when using the `ScpCfCloudPlatform`.

  Service bindings are now loaded with the following priority (highest to lowest):
...
- Fix `SapPassportAccessor.getPassportForNextRequest()` to no longer throw an exception if either the current `ThreadContext` is unavailable or the incoming HTTP request is missing the SAP passport header.
  Instead a new passport will be generated on behalf of the base passport template.
  If the base passport template is not defined a new template will be instantiated at runtime via `SapPassportFactory`.
- We removed the superfluous WARN log message that appeared whenever a new mTLS HTTP client was built.
