...
[Maven Central](https://search.maven.org/search?q=g:com.sap.cloud.sdk*%20AND%20v:3.38.0) | [Javadoc](still-missing)
...
- Introduce a new API on table parameters of `BapiRequest`.
  When a table uses a domain-type as row-type there are no field names to refer to.
  But such a table can be treated as one-dimensional vector of primitive values, e.g.:
...
- Add support for `SAMLAssertion` authentication type on Cloud Foundry.
  Refer the [help page](https://help.sap.com/viewer/cca91383641e40ffbe03bdc78f00f681/Cloud/en-US/d81e1683bd434823abf3ceefc4ff157f.html) for more detail on `SAMLAssertion` authentication type.
- Introduced a new API to build a `Destination` for connecting to services on Cloud Foundry.
...
- Introduce a new API to construct a `ScpCfDestinationLoader` that allows to override the default timeout for the Time Limiter resilience pattern.
  If not specified, requests will automatically timeout after 6 seconds.

- Add support for recursive tables and structures in parameters of `BapiRequest` API for both protocols: _JCo_ and _SOAP_. Example:
  ```diff
...
- Fix an issue with OData V4 update requests where the serialization of primitives with types like Integer, Boolean and Double leads to null values.
- Fix an issue with JCo serialization of BAPI requests where adding field parameter values of type `ErpType` led to an exception.
