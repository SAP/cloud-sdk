...
- Update the SAP Business Rules OpenAPI client library to the latest version of the Business Rules service.
  This includes a breaking change to the field `List<TextPredefinedResults> predefinedResults` which is renamed to `List<TextPredefined> predefined`.
...
- The `DefaultLocaleFacade` does no longer consider the `ServletRequest` (accessed via the `RequestAccessor`) to determine the desired `Locale`.
  ...
  As a fallback, in case the header cannot be found, the default `Locale` for the server will be returned - this has also not been changed.
...
- Fix an issue with OpenAPI client not being able to instantiate the class `ApiClient` when _Jackson_ formats other than _JSON_ are possible at runtime.
