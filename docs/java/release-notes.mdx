...
- [**Beta**] Reading destinations from environment variables now allows you to configure authorization token forwarding to the target system.
  ...
  **Note:** This means that token set via the `AuthTokenAccessor.executeWith(...)` will **not** be forwarded to the target system. 

- We now support adding headers to a specific destination using properties, so they will always be included when calling the target system. 
  Usage: `.property("URL.headers.foo1", "bar1")`. Relevant section in documentation: [SDK Docs](https://sap.github.io/cloud-sdk/docs/java/features/connectivity/sdk-connectivity-destination-service#provide-headers-for-destinations).
  
- We now support adding query parameters to a specific destination using properties.
  They will always be included in the query string when calling the target system.
  The query values should be provided without encoding. They will be encoded in the query string.
  Example usage: `.property("URL.queries.airport", "Köln Bonn")`. The query string will include `airport=K%C3%B6ln%20Bonn`. 
  Relevant section in documentation: [SDK Docs](https://sap.github.io/cloud-sdk/docs/java/features/connectivity/sdk-connectivity-destination-service#add-query-parameters-via-destination-properties)
  
## Improvements

- `HttpClientFactory` is now a `@FunctionalInterface`. With that, (ad-hoc) instances can conveniently be constructed using simply a lambda expression like so: `HttpClientFactory factory = destination -> HttpClients.createDefault();`.
- Removed unnecessary warning messages when not having an XSUAA or IAS service bound to the application.
...
- We fixed an issue where HTTP header keys of an OData response were not handled case-insensitive, causing problems with 
  processing those responses. This is fixed now and should handle headers the same way, regardless of capitalization.
