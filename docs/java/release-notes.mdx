...
## 3.36.0 - January 13, 2021

[Maven Central](https://search.maven.org/search?q=g:com.sap.cloud.sdk*%20AND%20v:3.36.0) | [Javadoc](https://help.sap.com/doc/55bb1642ea614bf582d514ed34401136/1.0/en-US/index.html)

### Known issues

- Currently, the TomEE based archetype for Cloud Foundry does not work with the latest SAP Java Buildpack when using Java 11. Please use the buildpack version `sap_java_buildpack_1_29` for this use case.

### Compatibility Notes

- Remove dependencies from SDK Bill-of-Material (BOM)
  - Netty
  - Protobuf
...
- Introduce support for String-based filter functions `endswith`, `startswith` and `substringof` in the OData V2 VDM.

### Improvements

- Replace the deprecated constant `org.apache.commons.codec.Charsets.UTF_8` in the HelloWorldControllerTest class of the Spring archetype by its recommended successor `java.nio.charset.StandardCharsets.UTF_8`. This resolves a compiler warning.
- Add Maven module `s4hana-api-odata-v4` into `s4hana-all` to enable type-safe OData v4 client library for SAP S/4HANA Cloud Edition.
- Dependency Updates:
...
- Fix an issue where OData V2 Update requests with PATCH contained incorrectly serialized date values. 
- Fix an issue where OData Batch requests sent an invalid `Accept` header.
- Fix an issue where OData Batch response API did not conveniently throw or wrap an exception for failing HTTP response code health checks. Affected methods:
...
- Remove a log message that incorrectly indicated an error when receiving OData responses without payload
