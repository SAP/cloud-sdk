...
- Add a new method (`excludingFields(...)`) to exclude specific fields from OData V2 and V4 `PUT` requests (i.e. `replacingEntity()`).
  This method can be used to to exclude read-only fields, as well as dependent properties that are not tied to key properties from your `PUT` request payload.
...
- Fix an issue with Destination initialization, where the `"Authorization"` header value is constructed with improper scheme capitalization: e.g. instead of `"bearer [...]"` it will now correctly consider `"Bearer [...]"` as header value for outgoing HTTP requests.
