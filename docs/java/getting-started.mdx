...
Note that the SAP Business Technology Platform Cloud Foundry environment environment only supports Java 8 out of the box but can be configured to also run with Java 11.
...
The mentioned archetypes work out-of-the-box for Cloud Foundry environment.
...
{ label: 'Cloud Foundry environment', value: 'cf', },
{ label: 'Neo', value: 'neo', }]}>
...
:::note Spring Support for Cloud Foundry environment
For Spring based projects we provide out-of-the-box support only on Cloud Foundry environment with archetype `scp-cf-spring`.
:::
...
- Get and configure the [SAP Cloud Foundry environment CLI](guides/cf-cli)
