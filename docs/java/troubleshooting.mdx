...
:::info **Symptoms**
Your OData request returns with a 400 (bad request) oder 500 (server error) HTTP code.
:::
...
- The service may not be implemented correctly or deviate from the OData standard / conventions.
  ...
  Check the request the SDK sends out (looking at HTTP logs) against the OData documentation.
...
- The Cloud SDK may send an incorrect OData request.

  Verify that the request the SDK sends out (looking at HTTP logs) does not conform to the OData documentation.
