...
:::info **Symptoms**
When running `Destination#asHttp()` you encounter exception message like `"DestinationAccessException: Failed to read authentication token. The destination service responded with an error: Could not verify the provided CSRF token because your session was not found"`.
:::
...
- The Destination Service on Cloud Foundry is unable to generate an authentication token that will be used in later requests.
  ...
  To solve that issue, please check every field in the destination configuration.
  The value for field "Token Service URL" may require the full service endpoint path, including `/oauth/token`
