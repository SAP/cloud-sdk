...
:::note Only needed if not using the SAP Cloud SDK Maven archetypes
You only need to add the following dependencies if your project is not based on one of the [SAP Cloud SDK Maven archetypes](https://search.maven.org/search?q=g:com.sap.cloud.sdk.archetypes).
:::
...
```
Moreover, ensure that you use the SAP Cloud SDK Bill-Of-Material to let the dependency versions be managed automatically.
