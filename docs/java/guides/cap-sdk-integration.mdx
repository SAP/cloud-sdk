...
Note that the `version` tags can be omitted, because the previously added the SAP Cloud SDK BOM controls the version of all the SAP Cloud SDK modules.
...
**Please note:** From CAP/CDS version `1.23.0`, the following step is no longer necessary.
...
**Please note:** From CAP/CDS version `1.23.0`, the additional wrapping using `threadContextExecutor.execute(..)` in the following example is not necessary but it is still required to wrap your calls with CAP's `RequestContextRunner`.

If your app uses asynchronous calls which spawn new threads or reuse pooled threads, the call must be decorated with the respective wrapper classes.
...
You need to use the `ThreadContextExecutor` from the SAP Cloud SDK (refer to the [ThreadContext documentation](https://sap.github.io/cloud-sdk/docs/java/features/multi-tenancy/multi-tenancy-thread-context/#what-is-a-thread-context)) in conjunction with the `RequestContextRunner` from [CAP](https://cap.cloud.sap/docs/java/provisioning-api#threading-requestcontext).
...
Callable<Object> task = () -> cdsContextRunner.run(context ->
...
Thus, when the parent thread dies the context will cease to exist and no longer be available in any of the threads.
