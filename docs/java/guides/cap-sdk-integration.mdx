...
Note that the `version` tags can be omitted, because the previously added Cloud SDK BOM controls the version of all SDK modules.
That is, you only need to increase the version of the BOM and all modules get updated automatically.
:::

:::tip
The two mentioned dependencies are modules that cover a broad range of possible Cloud SDK use cases.
...
Now the Cloud SDK is integrated with the CAP application.
