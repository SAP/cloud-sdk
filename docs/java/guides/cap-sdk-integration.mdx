...
```java
// Obtain the current request headers in the parent thread
Property<?> headers = ThreadContextAccessor.getCurrentContext()
        .getProperty(RequestHeaderThreadContextListener.PROPERTY_REQUEST_HEADERS).get();
...
final Callable<Object> task = () -> cdsContextRunner.run(context ->
                                        threadContextExecutor.execute(() -> {
                                            // Manually set them again into the child thread
                                            // All critical information (auth token, tenant, user) will be derived from the headers
                                            ThreadContextAccessor.getCurrentContext()
                                                .setPropertyIfAbsent(RequestHeaderThreadContextListener.PROPERTY_REQUEST_HEADERS, Try.success(headers));
                                            ...
                                        } ));
```
