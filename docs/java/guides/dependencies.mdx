...
Besides the SAP Cloud SDK Bill-of-Material `sdk-bom` there is also the SAP Cloud SDK Modules Bill-of-Material `sdk-modules-bom`, which manages the version of all SAP Cloud SDK modules only.
...
Please be aware that neither the `sdk-bom` nor the `modules-bom` will manage the versions of any Cloud SKD Maven plugins (like the `odata-generator-maven-plugin`) you may use.
