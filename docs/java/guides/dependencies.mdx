...
The [SAP Java Buildpack](https://github.wdf.sap.corp/xs2-java/xs-java-buildpack) brings some dependencies that are also brought by the SAP Cloud SDK.
...
We recommend to set a fixed Buildpack version in the `manifest.yml` and update it regularly.
...
In general, this is more safe than using always the latest Buildpack. 
...
Also see [this note](https://github.wdf.sap.corp/xs2-java/xs-java-buildpack#versioning) on Buildpack versioning.

:::note
There are no known dependencies in the [Community Java Buildpack](https://github.com/cloudfoundry/java-buildpack) that could interfere with the SAP Cloud SDK provided ones.
