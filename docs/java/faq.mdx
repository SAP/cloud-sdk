...
You can also create an issue on the Cloud SDK external GitHub repository or use the internal one if you're the SAP employee.
...
You can use pre-generated client libraries supplied with the SDK or generate your client from the SDK specification.
...
Some of them are available only for SAP internal use. Others, like the Workflow service on Cloud Foundry, are released publicly.
...
However, we do not recommend starting new projects with SAP Cloud Platform Neo as Cloud Foundry is better suited for apps and S/4HANA extension development.
