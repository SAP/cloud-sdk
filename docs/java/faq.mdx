...
The SAP Cloud SDK for Java currently does not fully work with the [Reactive Spring Technology Stack](https://docs.spring.io/spring-framework/docs/5.0.0.M1/spring-framework-reference/html/web-reactive.html).
Most importantly the [Multi Tenancy Features](features/multi-tenancy/multi-tenancy-thread-context) are not functional on that stack.
The SAP Cloud SDK for Java and its multi tenancy features are build on a servlet based concept which is incompatible with the reactive stack.
