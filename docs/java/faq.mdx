...
### How am I supposed to use the OData Attachment Service?
...
As a consequece the generated `AttachmentService` interface and `DefaultAttachmentService` implementation cannot be used reliably.

We recommend the following manual requests instead - without our convenience API:

1. Request CSRF token with HEAD request.
2. Upload Attachment with POST requset.
...
Once you've decided on `InputStream` or `File` as underlying data type for `HttpEntity`, you need to apply values for HTTP headers `x-csrf-token`, `Content-Type`, `Slug`, `BusinessObjectTypeName` and `LinkedSAPObjectKey`. The last three header values are **required** to be wrapped in single-quotes `'`.
