...
3. Create and bind the connectivity service with the `connectivty_proxy` plan.
   ...
   Retrieve the secret's content using the previous code snippet and convert it into a `JSON` before saving it. You can use the following as a guiding example.
   ```yaml
   apiVersion: v1
   kind: Secret
   metadata:
