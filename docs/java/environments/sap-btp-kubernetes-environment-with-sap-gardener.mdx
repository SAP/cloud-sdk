...
SAP Cloud SDK provides functionality to load service binding information when running the application in any Kubernetes environment.
This means that the SAP Cloud SDK features can be used in a Kubernetes environment without additional configuration (specifically `ScpCfCloudPlatform`).
Below you can find an architectural overview of the relevant classes.
**Please note that the API is still in an experimental state at this time.**
...
An interface offering only one method to load the existing service bindings for the current application.
...
Used for transforming file system structures (e.g. files and directories) into service bindings, starting at a specific root location based on the provided `ParsingStrategy`.
...
Used for transforming environment variables into service bindings based on the provided `ParsingStrategy`.
...
Used for merging the result of an arbitrary amount of given instances based on the provided `MergingStrategy`.
...
Used for wrapping another instance and caching the result for a certain amount of time.
...
We suggest that `ScpCfCloudPlatform` is used when running in a Kubernetes environment.
The above-mentioned architecture can be used by calling the `Map<String, JsonArray> getVcapServices()` method.
