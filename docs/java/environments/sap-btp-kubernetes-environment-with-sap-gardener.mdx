...
This guide assumes you have the [Kubernetes CLI](https://kubernetes.io/docs/tasks/tools/#kubectl) installed on your local machine and have it set up for cluster access.
...
:::tip
You can find more information on how to containerize Spring Boot applications in [this guide](https://spring.io/guides/gs/spring-boot-docker/) (in particular, check the *Containerize It* section).
:::
...
```bash
$ kubectl get deployment sdkjava-test-app-deployment

NAME                          READY   UP-TO-DATE   AVAILABLE   AGE
sdkjava-test-app-deployment   1/1     1            1           72d
```
...
Create a new YALM file containing the following Ingress configuration:

```yml title="ingress.yml"
---
...
4. Mount the `my-destination-service-secret` secret into the file system of the application as follows:
    1. Find the empty list of `volumes` at the end of your `deployment.yml`.
        ...
        - name: my-destination-service-binding-volume
          secret:
            secretName: my-destination-service-binding
        ```
