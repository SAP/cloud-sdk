...
3. Monitor the status of the deployment by running: `kubectl get deployment <name-of-the-deployment>`.

Eventually, you should see an output similar to:

```bash
kubectl get deployment <name-of-the-deployment>

NAME                           READY   UP-TO-DATE   AVAILABLE   AGE
<name-of-the-deployment>       1/1     1            1           15s
```
...
3. Monitor the status via `kubectl describe ServiceInstance <name-of-your-destination-service-instance>`.
...
kind: ServiceInstance
metadata:
  name: <name-of-xsuaa-service-instance>
spec:
...
kind: ServiceBinding
metadata:
   name: <name-of-your-xsuaa-service-binding>
spec:
   instanceRef:
     name: <name-of-xsuaa-service-instance>
...
We used a slimmed down version of this [repository](https://github.wdf.sap.corp/xs2/approuter.js) for this purpose.

1. Create a new YAML file describing the approuter deployment:
<!-- vale off -->
   ```yml title="deployment.yml" {18,32}
   apiVersion: apps/v1
   kind: Deployment
   ...
   spec:
     ...
     template:
       ...
       spec:
         ...
         - image: <name-of-the-image>
           imagePullPolicy: Always
           name: <name-of-the-approuter-app>
           env:
             - name: PORT
               value: "5000"
             - name: destinations
           ...
           volumeMounts:
             - name: <name-of-xsuaa-service-instance>
               mountPath: <mount-path-for-xsuaa-service-instance>
         volumes:
           - name: <name-of-xsuaa-service-instance>
             secret:
               secretName: my-xsuaa-secret
   ...
   spec:
     ...
     selector:
       app: <name-of-the-app>
   ```
2. Also adapt the `xs-app.json` file:
```json
{
    ...
    "routes": [
        {
            ...
            "identityProvider": "<identity-provider-url>",
            "csrfProtection": false
        }
    ]
}
```
3. Follow steps similar to steps 2-3 in [Create a Kyma Deployment section](#create-a-kyma-deployment) to deploy the approuter and check if the deployment is successful.

4. Update the already created XSUAA service instance with few parameters like `xsappname` and `oauth2-configuration`.
This is required to provide authenticated access to backend application via the approuter.
The updated `xsuaa-service.yaml`:
...
5. Update the configuration via `kubectl apply -f xsuaa-service.yaml`.

6. Make the approuter accessbile outside the cluster by creating a API rule:

```yml title="create-api-rule.yml"
...
spec:
  ...
  rules:
    - accessStrategies:
      ...
      path: /.*
  service:
    host: s4sdk-k8s-kyma-q2wk189v.<URL-pattern-for-outside-cluster-access>
    name: <name-of-the-approuter-app>
    port: 5000
```

7. Update the configuration via `kubectl apply -f create-api-rule.yml`.

8. Finally, access the application by using the host address provided in the API rule created and appending the <path-for-backend-endpoints> given in `xs-app.json` along with the an actual end point of your backend application.