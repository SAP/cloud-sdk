...
In fact, you may deploy any application that can be dockerized.
...
Once your Kyma environment is created, you can download a [Kubeconfig file](https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/) directly from the BTP cockpit.
...
This will let `kubectl` choose a random available port on your local machine everytime you run the command.
...
So you should be able to successfully run the following `curl` command:
...
1. Open your Kyma dashboard by clicking the link in your BTP cockpit:
