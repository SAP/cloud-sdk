...
The `@Controller(business-partners)` decorator marks the class `BusinessPartnerController` as controller (i.e. a thing that handles requests).
...
Next, we will create a service which will be responsible for fetching the business partners.
