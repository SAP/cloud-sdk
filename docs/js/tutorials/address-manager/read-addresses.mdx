...
We will extend the functionality of the API in this tutorial and the [follow up part](./change-addresses.mdx).
...
In real applications you would use a real S/4HANA system and the [destination service](../../features/connectivity/destination) to avoid leaking private or secret information through hard-coding system properties.
...
Since, we know the type of the property, the typed client will prevent you from passing values of the wrong type.
...
However, we are interested in the addresses related to one specific business partner object.
...
Again, we use the local mock server from the previous section for the `getAll` request.
