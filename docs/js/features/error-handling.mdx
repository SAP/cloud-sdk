...
Generally speaking, there is no difference in handling errors when working with the SDK.
However, some errors are caused by underlying errors.
In these cases the SAP Cloud SDK throws an `ErrorWithCause`, that allows access to the cause of this error and if there are multiple errors one causing the next, also to the root cause.
