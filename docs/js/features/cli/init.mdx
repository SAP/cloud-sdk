...
The project name is relevant for the Cloud Foundry environment repository.
...
When deploying to Cloud Foundry environment, it calls the entered command to start your application.
...
+--------------------------------------------------------------+
 ...
 - Push to Cloud Foundry environment (`cf push`)
...
After the initialization is finished, the CLI displays the necessary steps to deploy your application to Cloud Foundry environment.
...
According to the [Cloud Foundry environment documentation](https://docs.cloudfoundry.org/buildpacks/node/node-tips.html) your app needs to listen to the `PORT` variable.
...
Once your project is prepared, you can use the [Cloud Foundry environment CLI](https://docs.cloudfoundry.org/cf-cli/install-go-cli.html) to deploy your application.
You may need to log in to your Cloud Foundry environment account first.
