...
When deploying to Cloud Foundry, it needs to know the command to start your application.
This can be any bash command.
Often the command will be `npm run start`.

The start command will be entered in the `manifest.yml`.
If you don't know the command at this moment, you can adjust it manually in the `manifest.yml`.

#### Do You Want to Provide Anonymous Usage Analytics to Help Us Improve the SAP Cloud SDK?

If you give your consent, the SAP Cloud SDK will collect some anonymized information on your project and computer.
This helps us understand the ecosystem in which the SAP Cloud SDK is used and prioritize features accordingly.
The consent is maintained in the `sap-cloud-sdk-analytics.json`.
...
Those deployments should be done by a CI/CD pipeline that ensures successful tests before a deployment.
...
If you prefer different tooling, we recommend you set up the your project first and use the CLI to add the SAP Cloud SDK afterwards.
