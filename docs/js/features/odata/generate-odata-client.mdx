...
With the generator you can generate custom OData client libraries for OData services.
You can then access these services using the client libraries.
...
If you want to generate JavaScript code and install the generator globally, you have to install the `@sap-cloud-sdk/odata-v2` or `@sap-cloud-sdk/odata-v4` package in your project.
...
- `servicePath` - the URL path to be prepended before every request.
  This is read from the EDMX file if available, otherwise, the value here will be `VALUE_IS_UNDEFINED`. In that case, it should be adjusted manually.
...
The SAP Cloud SDK OData client generator for JavaScript/TypeScript generates TypeScript code.
By default, it transpiles the generated sources to JavaScript and provides a `package.json` for the client (this differs from the OpenAPI generator).
...
Therefore, you need a local reference to those packages for transpilation.
Those packages are not installed for you, so that you can decide where to install them (e.g. in your project or in the subdirectory of your client).
If you installed the generator globally, transpilation will fail unless you install the required package (`sap-cloud-sdk/odata-v2` or `sap-cloud-sdk/odata-v4`).
If you installed the generator as a `devDependency`, transpilation will work automatically.
However, for productive use, you may have to define the dependency productively.
You can suppress transpilation with the `--generateJs` flag.
