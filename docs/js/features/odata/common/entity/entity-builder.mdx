...
To build an entity by assigning its properties, you can use the entity builders provided by every entity's api class.
Use the `.entityBuilder` method to access the builder, set the properties and finally use the `.build` method to yield the entity.
...
const businessPartner = new BusinessPartnerApi().entityBuilder()
  .firstName('Peter')
  .lastName('Pan')
  .toBusinessPartnerAddress([
    new BusinessPartnerAddressApi().entityBuilder().country('Neverland').build()
  ])
  .build();
```
