...
In version 2.0 of the SDK, two new changes have been introduced:
- an API class for each entity (e.g. `BusinessPartnerApi`), providing access to its properties via a `schema`, and  
- a common utility method, e.g. `businessPartnerService()`, with accessors for all the API classes.  
