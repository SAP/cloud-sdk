...
The SAP Cloud SDK supports custom fields on your entites, that are not covered by the specification the according service is based on.
...
```ts
// add custom fields to the exisitng fields
