...
In order to work with an instance of the retrieved entity, you can use the `.as` method, that allows you to transform the raw data into an instance of the given constructor.
Note, that retrieve responses can be [`ErrorResponse`s](#error-handling).
Therefore, it is crutial to check responses for success, before casting them to `ReadResponse`.
