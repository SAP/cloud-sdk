...
They have to be combined in a changset, which in turn can be passed to the batch request.
...
The subresponse of a changeset request can either be `WriteResponses` or an `ErrorResponse`.
...
In order to work with an instance of an entity given in a `WriteResponse`, you can use the `.as` method, that allows you to transform the raw string body into an instance of the given constructor.
