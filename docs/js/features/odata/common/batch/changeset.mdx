...
Change request cannot be passed to a batch request directly.
They have to be combined in a changset, which in turn an be passed to the batch request.
Once a batch request is executed it returns a list of `BatchResponse`s.
Those contain the raw response information of each subrequest, the subresponse of a changeset request can either be `WriteResponses` or an `ErrorResponse`.
To determine if a request was successful use `.isSuccess()`.

Successful requests can be cast to `WriteResponses` which contains all subresponses for the changeset request.
Those reponses can be accessed by `.responses` and have the type `WriteResponse`.
Each `WriteResponse` contains the HTTP code and can contain the raw body and the constructor of the entity that was parsed from the response, depending on whether there was a response.
Create requests typically do not return a response.
In order to work with an instance of an entity given in a `WriteResponse`, you can use the `.as` method, that allows you to transform the raw data into an instance of the given constructor.
Note that the response cannot exist, so you should only call this method if you know that there is data.
...
Therefore, it is crutial to check responses for success, before casting them to `WriteResponses`.
...
Once the batch request is exeucted it returns a list of `BatchResponse`s, which in this example contains one response only, namely the one for the changeset.
