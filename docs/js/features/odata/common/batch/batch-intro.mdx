...
This can significantly reduce the network overhead you have to deal with when you want to execute a large number of requests.

An OData batch request can consist of a number of [retrieve requests](#retrieve-request) and [changesets](#changeset).
Those can be combined arbitrarily.
