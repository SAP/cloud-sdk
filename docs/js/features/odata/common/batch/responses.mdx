...
Requests that were not sucessful are reflected as `ErrorResponse`s.
Responses to successful requests are are represented as `ReadResponse`s for retrieve requests, and `WriteReponses` for changesets.
...
- `.isWriteReponses()` - to check that the response is either of type `WriteResponses`
