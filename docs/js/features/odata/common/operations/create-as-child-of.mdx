...
const existingBusinessPartner = await businessPartnerApi.requestBuilder()
  .getByKey(myID)
...
const newAddress = new BusinessPartnerAddressApi().builder()
  .country('DE')
  .postalCode('14469')
  .cityName('Potsdam')
  .streetName('Konrad-Zuse-Ring')
...
```ts
new BusinessPartnerAddressApi().requestBuilder()
  .create(newAddress)
  .asChildOf(
