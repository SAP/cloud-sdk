...
There are some valid use cases that the typed response does not meet special requirements like below:

- the response cannot be deserialized automatically, so the raw response can work as workaround
- instead of the response body, additional information is needed like status code or response header
- in addition to the response, the request built by the SAP Cloud SDK needs also to be returned

To fulfill the requirement shown above, the SAP Cloud SDK provides the `executeRaw` function that returns the original request and the corresponding raw response.

:::note typed response from `execute()` is the best practice
In general, to make a request, using `execute` function for returning a typed response is the best practise.
The `executeRaw` should only be used for good reasons.
:::
