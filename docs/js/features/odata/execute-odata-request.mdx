...
<ul style={{ 'margin-top': '-16px' }}>
  <li>
    In rare cases, when the response data cannot be deserialized. This can
    happen when function imports use an entity type as return type, and this
    entity type is shared by multiple `EntitySet`s. Without further information,
    it is unclear which `Entity` should be deserialized. For those cases, you
    have to use `executeRaw` and use the `deserialize` function on the response
    data.
  </li>
</ul>
