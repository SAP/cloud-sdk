...
```ts
import { MyEntityApi } from './outputDir/my-service';

return new MyEntityApi().requestBuilder()
  .<requestType>(...)
  .<additionalRequestConfiguration>(...)
  .execute(destination);
```

When using a custom OData client, import an entity's api class from the generated service directory (`my-service`) within the specified output directory (`outputDir`).
...
```ts
import { PredefinedEntityApi } from '@sap/cloud-sdk-vdm-predefined-service';
```
...
```ts
new MyEntityApi().requestBuilder().getAll().addCustomHeaders({
  apikey: 'my-api-key'
});
```
...
```ts
new MyEntityApi().requestBuilder().getAll().withCustomQueryParameters({
  language: 'en'
});
```
...
```ts
new MyEntityApi().requestBuilder()
  .getAll()
...
```ts
const httpResponse: HttpResponse = new MyEntityApi().requestBuilder()
  .getAll()
  .executeRaw(destination);
```
