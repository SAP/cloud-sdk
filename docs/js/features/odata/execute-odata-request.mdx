...
<ul style={{ 'margin-top': '-16px' }}>
  <li>
    ...
    This can happen when function imports use an entity type as return type, and this entity type is shared by multiple `EntitySet`s.
    Without further information, it is unclear which `Entity` should be deserialized.
    For those cases, you have to use `executeRaw` and use the `deserialize` function on the response data.
    <!-- prettier-ignore-end -->
  </li>
</ul>
