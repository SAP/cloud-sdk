...
When using a custom OData client, import the common service method, e.g. `myEntityService()`, from the generated service directory (`my-service`) within the specified output directory (`outputDir`).
...
```ts
import { predefinedEntityService } from '@sap/cloud-sdk-vdm-predefined-service';
```
