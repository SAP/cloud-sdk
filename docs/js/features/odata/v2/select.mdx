...
So there will be a field for each property. E.g. the businessPartnerApi has `schema.FIRST_NAME` as a representation of a property and `schema.TO_BUSINESS_PARTNER_ADDRESS` as a representation of a navigation property.
