...
If you configured your target system in such a way, you can use an environment destination to stop the destination lookup right away.
...
Note that this approach is not possible for multi tenant scenarios, because the destination services takes care
