...
With the `registerDestination` function it is possible to put destinations into the environment variables. You can then use these destinations in the same way as those retrieved from the SAP BTP.
