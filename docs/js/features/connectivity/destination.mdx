...
These destinations are called `instance destinations` since they are tied to a service binding called instance on BTP.
...
Other headers or query parameters built by the SDK are overwritten by additional headers and query parameters on the destination.
