...
- `iasToXsuaaTokenExchange`: Switches on token exchange form IAS format tokens to XSUAA if needed using the `@sap/xssec` library.
...
- `enableCircuitBreaker`: Switches on circuit breakers to protects the calls to the `XSUAA` and `destination-service`.
  Default value is true.
- `timeout`: Sets the timeout for the calls to SAP BTP services like `XSUAA` and `destination-service`.
  ...
  There is a second timeout options on the request level, setting the timeout for the calls to the destination target.

## Destination Properties

The destination object may contain additional properties.
The properties change the behavior how the SAP Cloud SDK handles the HTTP request at runtime.

### SAP Client

The property `sap-client` is considered by the SAP Cloud SDK.
This property leads to a header field `sap-client` with the specified value in the HTTP request.
...
There are two destination property used to establish trust to systems using certificates not issued by trusted certificate authorities:
...
<img
  alt="Truststore location property on destination"
  src={useBaseUrl('img/destination-property-truststore.png')}
  class="center"
/>

<br />
Please use the `TrustAll` with great caution since it opens the gate to man in the
middle attacks.

<img
  alt="TrustAll property on destination"
  src={useBaseUrl('img/destination-property-trust-all.png')}
  class="center"
/>
