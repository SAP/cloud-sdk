...
:::note
This section is written for version 2.0 of the SAP Cloud SDK.
Most of the options are also available in version 1.0 but not all of them e.g. `timeout.
:::
...
- `destinationName`: The only mandatory property.
...
- `selectionStrategy`: Strategy which account is searched for a destination.
  Default is `subscriberFirst`.
  Alternative values are `alwaysProvider` and `alwaysSubscriber`.
- `iasToXsuaaTokenExchange`: Boolean value with default true.
  Performs a token exchange form IAS format tokens to XSUAA if needed using `@sap/xssec` library
- `cacheVerificationKeys`: Boolean value with default true.
  The verification certificates for the JWT are cached if enabled.
- `useCache`: Boolean value with default false.
  Stores received destinations in the cache.
...
- `enableCircuitBreaker`: Boolean value with default true.
  Protects the calls to the `XSUAA` and `destination-service` by a circuit breaker.
- `timeout`: Value in milliseconds with a default of 10000.
  Sets the timeout for the calls to the `XSUAA` and `destination-service`.
