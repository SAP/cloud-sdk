...
A simple request to receive some business-partners from an S/4HANA system would look like:
...
The SDK provides a `mockDestinationsEnv(...destinations)` function to set the `destinations` environment variable for the current process programmatically.
...
In order to access the destination service, the SDK will first fetch an access token from the XSUAA service via one of supported authentication flows:
...
:::note
**OAuth2UserTokenExchange** is currently not supported. We're working on it.
:::

The token retrieved from XSUAA service is used to make a call to the destination service and receive the destinations.
...
In order to control this fallback behavior a selection strategy can be passed to the destination lookup:
