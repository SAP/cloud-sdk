...
For every header or query parameter create on additional property and replace `<header-key>` and `<query-key>` with the name of your header or query parameter and set the according value.
