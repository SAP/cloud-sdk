...
Most of the options are also available in version 1.0 but not all e.g. `timeout.
:::
...
- `iasToXsuaaTokenExchange`: Switches on token exchange form IAS format tokens to XSUAA if needed using `@sap/xssec` library.
...
- `timeout`: Sets the timeout for the calls to SAP BTP services like `XSUAA` and `destination-service`.
  Value is in milliseconds with a default of 10000.
