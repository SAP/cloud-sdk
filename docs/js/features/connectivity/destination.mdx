...
If you use JWTs not issued by the XSUAA you can configure validation by the destination service using the `x_user_token.jwks` or `x_user_token.jwks_uri` property.
For more details on the JWTs have a look at the more detailed guide(../../guides/how-to-retrieve-jwt.mdx).
