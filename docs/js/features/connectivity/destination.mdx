...
Most applications developed on SAP Business Technology Platform (SCP) will integrate in some way with other line of business solutions and systems.
...
On SAP BTP this abstraction is provided by a so-called destination object.
...
The SAP Cloud SDK helps you receive this object and provides also options for local testing outside the SCP.
...
`yourDestination` is a [`Destination` object][Destination interface].
...
[Destination interface]: <pathname:///api/1.32.0/interfaces/sap_cloud_sdk_core.destination>
...
The value is expected to be a stringified JSON array, where the items adhere to the [`Destination` interface][Destination interface].
...
It takes a [`Destination` object][Destination interface], transforms it to a JSON object, and adds it to `process.env.destinations`.
