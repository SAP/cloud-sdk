...
The SDK supports [OData](../odata/overview.mdx) and [OpenApi](../openapi/overview.mdx) services.
For both service types the `execute()` methods triggers the request to the target system:

```ts
//OData
const responseOData = await BusinessPartner.requestBuilder().getAll().execute(myDestination);
//OpenApi
...
All statements apply to OData and OpenApi alike.
