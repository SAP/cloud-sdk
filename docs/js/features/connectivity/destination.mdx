...
On BTP this abstraction is provided by a so-called destination object.
...
This option is present for deployment and testing in a local environment outside the BTP where no destination services are available.
...
A customer is represented by an account on BTP and a service considering the account is a `tenant aware service`.

Tenant aware services on BTP are offered to customers via a `subscription` which works on a high level as follows:
