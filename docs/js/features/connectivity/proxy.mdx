...
We pre-generate type-safe clients for SAP S/4HANA APIs and publish them to [npm](https://www.npmjs.com/search?q=%40sap%2Fcloud-sdk-vdm-*) for your convenience.
...
This example uses the **BusinessPartner** service from the SAP S/4HANA suite.
...
The SAP Cloud SDK does a lot of heavy lifting under the hood to take care of complexity so that you can fully concentrate on your app's logic.
...
**To name a few things handled by SAP Cloud SDK for JavaScript:**
...
1. A local proxy running on the cloud platform that connects to SAP S/4HANA OnPremise systems.
...
If you provide the destination name the SAP Cloud SDK will try to lookup as described [here](destination.mdx).
...
If `OnPremise` is selected it is assumed that this destination points to an SAP S/4HANA OnPremise system.
...
From there the request will pass via the **SAP Cloud Connector** to the SAP S/4HANA OnPremise system.
