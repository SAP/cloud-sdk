...
:::caution
This is an SDK version 2.x feature.
:::

There are some use cases, when you want to define the priority of headers and parameters:
- I want to test with my hard coded api key/JWT and ignore the ones from e.g. the destination
- I want to have a default query parameter, which can be overwritten by the destination properties

Check the example below, so you can define the headers and parameters with origin information:
