...
- You can also use an existing application with bound connectivity service as a ssh tunnel:
    - Enable ssh for the application as described [here](https://docs.cloudfoundry.org/devguide/deploy-apps/ssh-apps.html)
    - Open a ssh tunnel with forwarding via the ` cf ssh MY-APP -L LOCAL-HOST:LOCAL-PORT:REMOTE-HOST:REMOTE-HOST-PORT` command.
