...
For quick local test purpusoes a deployment to Cloud Foundry is tedious and the following workarounds are possible:
- [SAP Business Application Studio](../../guides/bas-external-system.mdx) has some build in functionality to connect to Cloud Foundry.
- If a VPN is accessible from where the onPremise system is reachable use a [environment destination](./destination.mdx) to access the system directly.
