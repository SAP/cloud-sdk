...
The Cloud Connector works like a reverse proxy opening a connection between some allowlisted service endpoints of the On-premise system, and the connected Cloud Foundry environment account.
Only applications running in the connected Cloud Foundry environment account can access the services.
...
The proxy is only reachable from the Cloud Foundry environment space, so you cannot use that flow for applications running outside of Cloud Foundry environment.
...
and will be used by the [HTTP client](generic-http-client.mdx) of the SAP Cloud SDK to execute the request.
