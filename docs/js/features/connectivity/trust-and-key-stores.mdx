...
description: This article describes how configures trust and keystores in the SAP Business Technoligy Platform and how the SDK handles them..
keywords:
  - sap
  - cloud
  - sdk
...
1. You are actually talking to the person you think you talk
2. The content of your conversation remains private ensure by some encryption
...
- You can encrypt something with one key, but only with the counterpart you can decrypt it.
- You can distribute one key but you can not derive the other part from the distributed one.
...
The format of the private and public keys are certificates which provide additional information like:
...
We already introduced a few terms like certificate for the public and private key.
However, we would like to introduce a few terms:

- `Keystore`: A keystore is an object containing keys.
  In most cases these are private and public key pairs as introduced above.
  You need a password to open a keystore.
  Note that a keystore may also contain just a certificate but in general the term is used for the object containing pairs.
  Typical formats are `.JKS`, `.P12`, `.PFCKS` and `.PKCS7`
- `Truststore`: A truststore contains public keys i.e. certificates.
  Depending on the format also a truststore is protected by a password.
  Typical formats are `.JKS`, `.CRT`, `.PEM` and `.PUB` and `.DER`.

For X.509 certificate there are two encodings base64 ASCII (e.g. `.PEM`) and binary (e.g. `.DER`).
The content of a PEM file looks like:
...
The server does not really care about the identity of the client.
...
- Client checks that the certificate issuer is in the listed of trusted [certificate authorities (CA)](https://en.wikipedia.org/wiki/Certificate_authority).
- Client checks that the content was actually signed with the public key.

If all checks have passed, the server is trustworthy.
Creating a secure connection is possible using also the private and public keys.
For a encrypted connection you need to interchange a secret.
...
You would store the public part of your client certificate in the server.
When sending a request you then access your keystore and sign your request with the private part.
This is called `client certificate authentication` or two way SSL because the identity of server and client is verified using private and public keys.

## Truststore Configuration

In most cases your server will use a certificate from a trusted CAs and one way SSL just works without additional actions.
Sometimes this is not possible and locally created key pairs also call `self signed certificates` are used.
...
- Maintain the `TrustStoreLocation` property on the destination and [select the uploaded certificate](https://help.sap.com/docs/CP_CONNECTIVITY/b865ed651e414196b39f8922db2122c7/3f1247537c1a4f069235ee63633659c5.html).
  The password value is only relevant for java keys store format (JKS).
...
Avoid this options if possible.
:::

## Keystore Configuration

As discussed above you can also use certificate to authenticate the client.
This is the mentioned two way SSL situation and is called `clientCertificateAuthentication` in the destination service.
The SAP Cloud SDK supports this authentication flow.
You have to do the following steps:

- Upload the keystore in destinations section of the subaccount
- Configure the destination to use [ClientCertificateAuthentication](sap destination service TrustStoreLocation) and provide the keystore information like keystore name and password.

:::caution
The SAP Cloud SDK for JavaScript supports only `.P12` and `.PFX` as formats for the keystore.
:::
