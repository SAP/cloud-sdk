...
- Provided by user on request level.
...
Also all choosen options like filter, select, or top in the OData clients lead to query parameters considered to be `internal`:
...
In case you have the need to encode all parameters also the custom ones, we exported a `encodeAllParameters`method to do that.
