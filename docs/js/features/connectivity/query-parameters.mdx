...
In this document we explain where how query parameters

## Header Origins and Priority

As partially described in the [destination guide](./destination#additional-headers-and-query-parameters-on-destinations) there are multiple origins for query parameters:

- Provided by user on request level. These parameters are referred to as `custom` parameters.
...
Note the difference between version 1.0 and 2.0 of the SAP Cloud SDK.
In the newer version we allow to specify the origin of the header, whereas for version 1.0 it is `custom` per default.
...
Also all filter, select, expand,top... options you choose in the OData clients lead to query parameters considered to be `internal`:
...
myEntityApi.requestBuilder().getAll().
    ...
    .top(5)
//request url: <host>/myEntityApi?$select=FirstName&$top=5&$format=json
```
...
:::info
This section applies to version 2.0 of the SAP Cloud SDK.
For version 1.0 encoding is done one OData related query parameters without a way to configure it.
:::
...
- `Custom` parameters are not encoded. The idea behind this is to give you a way to send parameters with highest priority to the target system as they are.
...
```
const parameterEncoder = function(params: Record<string,any>):Record<string,any>{
//your custom implementation
}
executeHttpRequest(destination,{parameterEncoder})
```

The `parameterEncoder function is applied to the keys and values of all parameters of the request. In case you have the need to encode all parameters also the custom ones, we exported a `encodeAllParameters`method to do that. Simply pass this method as the`parameterEncoder`to the`executeHttpRequest` call.

Note that this option is not available for the OData and OpenApi clients, because the typed clients take care of encoding already and an additional encoding step would easily lead to errors.
