...
This reduces the number of call to the destination service.
All the discussed options apply to the `execute()` and `getDestination()` method of the SAP Cloud SDK.
The destination caching is disabled by default and you switch is on by the `useCache` flag:
...
This is a save choice ensuring that you do not run into privilege escalation due to caching.
...
In such a case you can manually enforce a weaker isolation:
 ...
 ```ts
 .execute({destinationName: 'myDestination', jwt: 'yourJWT', isolationStrategy: IsolationStrategy.Tenant})
 ```
 ...
 ```ts
 .execute({ destinationName: 'myDestination', jwt: 'yourJWT' }, { useCache: true, isolationStrategy: IsolationStrategy.Tenant })
 ```
...
If you manually reduce the isolation be sure that your destination flow is really not propagating a user to the target system.
A erroneous configuration can lead to severe permission escalation between users.

:::Attention
In version 1.X of the SAP Cloud SDK the isolation strategy is always TenantUser per default and you need to set it manually.
Also you need to switch on the cache explicitly even if you pass an isolation strategy.
:::
...
The method behaves similar to the destination-service cache discussed above.
...
The chosen isolation depends on the given `JWT`:
- No `JWT`: Isolation is `Tenant` and value for the tenant is the provider account.
- `JWT` without `user_id`: Isolation is `Tenant` and value for the tenant is the `zid` of the `JWT`
- `JWT` with `user_id`: Isolation is `TenantUser` and values are taken from `zid` and `user_id`.

Also here you can overwrite the choice of the SAP Cloud SDK.
This could be necessary if you want to register a user-independent destination per tenant but your `JWT` contains a `user_id`.

```ts
registerDestination(destination,{jwt:'yourJWT', isolationStrategy: IsolationStrategy.Tenant});
```
