...
For such cases where a different cache storage is desired, the SAP Cloud SDK allows you to plug-in your own implementation for destination cache.

To create your custom destination cache provider, implement the `DestinationCacheInterface` interface and define the `get`, `set` and `clear` methods.
...
To do this, call the method `setDestinationCache` at the start of your application code, i.e., before the first calls to either the `getDestination` or the generic client's `executeHttpRequest` methods.  
Pass your cache provider instance as an argument to this method.

```ts
setDestinationCache(new CustomCacheProvider({ expirationTime }));
```

All subsequent calls to fetch destinations will use this cache.
