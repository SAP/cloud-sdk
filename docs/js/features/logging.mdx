...
If `VCAP_SERVICE` variables are present in the environment variables the `kibana` format is used, because the SAP Cloud SDK assumes your application is running on Cloud Foundry environment.
