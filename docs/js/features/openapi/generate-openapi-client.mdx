...
We recommend not to use a globally installed generator, because this hides the used generator version and causes problems for the transpilation to JavaScript code.
If you must use a globally installed generator, you have to install the`@types/node` and `@sap-cloud-sdk/openapi` package in your project in order to make the transpilation work.
