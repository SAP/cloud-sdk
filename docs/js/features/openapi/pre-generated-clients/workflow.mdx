...
They are separated into two services per environment: the Inbox service (OData) and the Worfklow service (OpenAPI).
The Inbox service is mainly meant to build a personal inbox, while the Workflow service provides APIs for managing workflows and tasks accross recipients.
...
- an existing instance of the Workflow service
...
For this examplary use case, assume a simple workflow definition:
...
The following use case shows how to initiate a workflow, retrieve its tasks, complete a task and retrieve the execution logs for this workflow.
...
The form to fulfill this task has one boolean field, `approve`:
