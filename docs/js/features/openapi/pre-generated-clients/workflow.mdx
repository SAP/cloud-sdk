...
They are separated into two services per environment: the Inbox service (OData) and the Workflow service (OpenAPI).
The Inbox service is mainly meant to build a personal inbox, while the Workflow service provides APIs for managing workflows and tasks across recipients.
...
For this exemplary use case, assume a simple workflow definition:
...
First, import the `WorkflowInstancesApi` from the Workflow service client.
...
Use the `startInstance` function build a request to start a workflow instance for the definition with the id `'myWorkflowDefinitionId'` and execute it against a [destination](../../connectivity/destination.mdx):
