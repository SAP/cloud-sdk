...
description: This article describes what the Workflow API for Cloud Foundry is and how
keywords:
  - sap
  - cloud
  - sdk
...
The [SAP Workflow services](https://help.sap.com/viewer/product/WORKFLOW_SERVICE/Cloud/en-US) allow you to automate business processes across organizations and applications on SAP Business Technology Platform, both Cloud Foundry and Neo environment.
They are separated into two services per environment: the Inbox service (OData), mainly meant to build a personal inbox and the Workflow service (OpenAPI), that provides APIs for managing workflows and tasks accross recipients.

The first SAP Cloud SDK pregenerated OpenAPI client is for the OpenAPI Workflow service.
There is no pregenerated client for the Inbox service, yet.
The remainder of this document will refer to the OpenAPI service for Cloud Foundry only.

:::note Prerequisites
The following documentation exlpains how to use the pregenerated client for:
¸

- an existing workflow service
...
To learn how to setup your environment to use the Workflow services, please refer to the [Workflow service documentation](https://help.sap.com/viewer/e157c391253b4ecd93647bf232d18a83/Cloud/en-US/fc3d44872c354742afd672aa8d9c16b4.html).
:::

## Installation

The client for the Workflow service is available as an npm package.
To install it, run:
...
For this examplary use case, we assume a simple workflow definition.

<IMAGE>

It contains one task definition to approve a request through a form with one boolean field, `approve`.
In the following we we show how to start an instance of a workflow, retrieve tasks of this workflow, update and complete a task and retrieve the execution logs for all this.

### Start a Workflow

First, import the `WorfklowInstancesApi` from the Workflow service client.
This API includes functionality to manage worfklow instances.
It allows you to start an instance of a worfklow for an exsiting defintion.
Use the `startInstance` function build a request to start a workflow instance for the defintion with the id `'myWorfklowDefinitionId'` and execute it against a [destination](../../connectivity/destination.mdx):
...
function startWorkflow(): Promise<WorkflowInstance> {
  return WorkflowInstancesApi.startInstance({
    definitionId: 'myWorfklowDefinitionId'
  }).execute(destination);
}
```
...
It contains information on the started workflow, e.g., `id`, the id of the instance, `defintionId`, the id of the definition, and status, which in this case is `'RUNNING'`.
...
It has properties like the id of the instance and definition (`id` and `definitionId`), the `status`, `priority`, etc.

### Update the Status and Context of a Task

To update a task, use the `updateInstance` on the `UserTaskInstancesApi`.
Pass the the mandatory instance id as first parameter.
...
The form to fulfill this task has one boolean field, `approve` as shown in the image above.
...
As in this example this is the only task, this will also complete the workflow.
Execute the request against a destination:
...
Every workflow tracks information on the events that happend throughout its execution.
...
function getLogs(
  workdlowInstance: WorkflowInstance
