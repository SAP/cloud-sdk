...
Please keep in mind, that these updates can have breaking changes as per the [semver spec](https://semver.org/#spec-item-4).
...
- **`manifest.yml`**: The deployment descriptor file for `SAP BTP, Cloud Foundry environment environment`.
...
## Deploy the Project on Cloud Foundry environment

### Prerequisites

The Cloud Foundry environment CLI comes in handy when you want to deploy your application to the SAP Business Technology Platform.
You can find installation instructions for all common platforms in the [`Cloud Foundry environment documentation`](https://docs.cloudfoundry.org/cf-cli/install-go-cli.html).
...
To deploy our application, we first need to login to Cloud Foundry environment in SAP Business Technology Platform using the `cf` CLI.
...
Dependencies will instead be installed automatically when deploying to `Cloud Foundry environment`.

The Cloud Foundry environment CLI will automatically pick up the `manifest.yml` of the project when deploying your application.
...
- The specified `path` instructs Cloud Foundry environment to upload all the files from the `deployment/` folder.
...
This tells `Cloud Foundry environment in SAP Business Technology Platform` to create an instance of the destination service with service plan `lite` and make it accessible under the name `my-destination`.
...
The value for `xsappname` again has to be unique across the whole of `Cloud Foundry environment in SAP Business Technology Platform`, so make sure to choose a unique name or prefix.
