...
The SDK can provide JavaScript libraries and project templates.
...
- **`Jenkinsfile`**: Jenkins pipeline definition file for quality assurance. It uses the [project "Piper" General Purpose Pipeline](https://sap.github.io/jenkins-library).
...
The Cloud Foundry CLI comes in handy when you want to deploy your application to SAP Cloud Platform.
...
We recommend to use a package manager for that. If you are using `chocolatey` on Windows, please find the instructions [here](https://chocolatey.org/packages/cloudfoundry-cli).
...
In order to deploy our application, we first need to login to Cloud Foundry in SAP Cloud Platform using the `cf` CLI.
...
For `Name`, choose a name that describes your system. For the example, we will go with `S4_SYSTEM`.
...
This will make the fields `User` and `Password` appear. Enter here the credentials of a technical user for your SAP S/4HANA Cloud system.
