...
You can always generate a typed client library youself by using our [OData](features/odata/generate-odata-client.mdx) and [Open API](features/openapi/generate-openapi-client.mdx) code generators. To search for supported services use our API Business Hub integration.
...
## 1.28.2 [Client Libraries] - Novevmber 17, 2021

### Compatibility Notes

- We stoped generating API documentation for typed client libraries.
You can discover all required API infomation via you IDE features like autocomplete and IntelliSence.
...
- If `JWT` has an expiration time we'll use it to override default cache expiration time of 5 minutes.
