...
## Case 1: Connection to an OnPremises System
...
### Prerequisite 1: The Business Application Studio
...
### Prerequisite 2: Cloud Connector and Destination Setup <a name="CCandDestSetup"></a>
...
Before you start the implementation it is good to test the connection.
The connectivity proxy running in BAS expects a pattern of the kind `http://<destinationName>.dest`.
When such a pattern is found the destination name is extracted and checked against the destinations in the related cloud foundry account.
...
All authorization information from the real destination are used.
...
So if your destination is called `S4H_Test.dest` and you want to query the catalogue service the command would be:

```bash
curl -v -i "S4H_Test.dest/sap/opu/odata/iwfnd/catalogservice;v=2/ServiceCollection?%24top=1"
```

If this returns 200 for the service you want to use, you can start the implementation.
The connectivity proxy has a build in cache which needs to be emptied after you adjusted destinations.
...
As discussed in the beginning, we need to set the `destinations` environment variable.
...
As you see you use the same pattern `<name>.dest` we discussed in the [testing section](#testing).
...
We have tested the flow with destinations using `BasicAuthorization`.
The SAP Cloud SDK team is currently investigation with the colleagues from BAS if principal propagation is possible and which conditions are needed.
...
Then select the application from which you wan to mirror the variables and select the folder containing the `.env` file to store it.
...
This is possible in principle, but difficult withing the local development environment.
We recommend to create dedicated development destinations with authentications flows like:
...
Note that for the connection to cloud system the destinations do not need the two properties `WebIDEEnabled` and `HTML5.DynamicDestination`, because the BAS connectivity proxy is not needed.
