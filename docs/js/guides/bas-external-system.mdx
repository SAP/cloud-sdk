...
The Business Application Studio (BAS) is a development environment offered as a service on the SAP BTP Cloud Foundry environment (CF).
...
You have an SAP BTP Cloud Foundry environment account and a subscription to the BAS.
...
On Cloud Foundry environment, the access to services is granted via the `VCAP_SERVICE` variables.
We assume that you have configured your application so that it can access the destination service.
If not, follow the [steps in the Cloud Foundry environment tutorial](https://developers.sap.com/group.s4sdk-js-cloud-foundry.html).

### Implementation

You can mirror the `VCAP_SERVICE` variables from your Cloud Foundry environment account to your local BAS.
