...
For calls to Cloud Systems i.e. systems which are rechable via the internet the local BAS proxy is not needed.
...
An exception to this rule are destinations using `ClientCertificateAuthentication` because the used http-agent does not yet [allow for certificate forwarding](https://github.com/TooTallNate/node-https-proxy-agent/pull/111).
As a work around you can add the target system to the `no_proxy` environment variable:
