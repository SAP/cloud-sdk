---
title: Connecting to External Systems From the Business Application Studio With the SAP Cloud SDK for JavaScript / TypeScript
...
The SAP Cloud SDK has a [destination lookup](../features/connectivity/destination-js-sdk) priority considering environment variables first.
...
**Case 1:** When run locally, the SAP Cloud SDK reads the destination from the environment variables. The `jwt` is irrelevant.
