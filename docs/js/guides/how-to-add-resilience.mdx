...
description: This article describes how you can add some layer of resilience to the SAP Cloud SDK
keywords:
  - sap
  - cloud
  - sdk
...
In contrast to Java, for JavaScript it is not so clear which framework is the de-facto standard the handle resilience.
...
We introduced a circuit-breaker for all calls going to SAP BTP services like XSUAA and destination-service.
...
Also for this pattern there are libraries available like [async-retry](https://www.npmjs.com/package/async-retry) which makes every asynchronous method perform some retries if they do not resolve.
...
- Distribution: How the retires are distributes over time.
  ...
  Also adding some random time deviation is distributing the load of parallel retires.
- Bail: A option to stop the retry for certain failure cases is useful in many cases.
