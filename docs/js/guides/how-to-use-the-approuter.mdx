...
We will show you how to secure your application and utilize multi-tenancy for principal propagation with a nestJS example.

## SAP Application Router
When we want to enable multi-tenancy for our application, we use the SAP Application Router.
...
If the request contains a valid JWT, the application router forwards the request to the target service; if the request does not contain a valid JWT, the user must authenticate himself.
Since we use an Identity Provider (IdP) to authenticate ourselves, the request is redirected to an IdP and then redirected back to the application router. 

### Approuter Setup
To deploy our approuter in SAP BTP Cloud Foundry, we need to configure it first.
...
We can optionally also specify a specific `identityProvider` which is used for the authentication.

In the example below, we forward every request against the approuter's `/` route to the backend destination's `/` route.
```json
{
...
In the `package.json` we only have one dependency, the approuter module.
```json
{
  "name": "approuter",
  "dependencies": {
...
The manifest contains our approuter, as well as environment variables which our approuter needs for multi-tenancy.
As you see under `env`, we specifify the `TENANT_HOST_PATTERN` and `destinations`.
...
We also have to bind the xsuaa which we configured with our `xs-security.json` to the approuter.
...
const xsuaa = getServices({ xsuaa: { name: 'approuter-scaffold-xsuaa' } }).xsuaa;
