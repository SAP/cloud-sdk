...
Similar to the SAP BTP CloudFoundry way of propagating a principal to handle multi-tenancy, you can also use the Approuter in Kubernetes.
...
4. Finally, configure the Ingress to be aware of the session-cookie created by the Approuter and point at the Approuter service.
   ...
   For that you just need specifiy the service names in your approuters `xs-app.json` and remove the previous rules you created for these endpoints in the Ingress.
