...
1. First we decide what kind of environment we are creating. Is it a PoC, a Staging, or a Productive environment?
2. Then we prepare and set up our cluster with the sufficient services and service bindings to satisfy the environment's requirements.
3. Finally, we adapt our application to deploy to Kubernetes and make sure it can consume bound services from within our Kubernetes cluster.

Select an environment you're planning to build and click on it to jump to a corresponding section:
...
Now let's take a look at how we adapt our application to deploy it to Kubernetes and make it consume services from inside of a Kubernetes cluster.  For this jump over to the [Deploy to Kubernetes](#deploy-to-kubernetes) section.
...
To make the services available in the Kubernetes cluster we need to register a service broker into the Service Catalog of our Kubernetes cluster.  In SAP BTP Cloud Foundry the role of a service broker is fulfilled by the `Service Manager`.
...
- Once you have the data from the Service Manager's binding, provide it to `helm` and deploy the service operator. Helm is a package manager for Kubernetes.:
...
### Example application
Here's the example application we're going to use: [Kubernetes app](https://github.tools.sap/cloudsdk/k8s-e2e-app) app.
...
In order to automatically deploy your application into the K8s cluster, two things are needed:
...
After completing the previous steps, run the command below. It shutdowns all the Pods in order to restart them.
```bash
...
The first one is our default domain, it's limited to a maximum of 64 haracters.
...
In case your application may doesn't have built-in authentication, you can add basic authentication in front of your Ingress.
...
Notice how `ingress-gate-auth` is the name of the secret containing our password. The text following the annotation `nginx.ingress.kubernetes.io/auth-realm:` contains a message, which is prompted when asking for credentials for the basic authentication.
Your basic authentication should now work. Accessing any path should open an authentication window.
...
3. Create and bind the connectivity service with the `connectivty_proxy` plan. We already explained how to do it above.
If you use the PoC environment, save the binding as a secret, use the example below to guide you.
Otherwise, you need to retrieve the binding first and then format it in `JSON` to save it as a secret.
...
:::info
Note that we used the `stringData` instead of `data` field to benefit from automatic base64 encoding instead of doing it ourselves.
...
6. Create a `values.yml` containing the configuration that suits your desired operational mode, for the available operational modes refer to the [documentation](https://wiki.wdf.sap.corp/wiki/display/NGP/Operational+modes#Operationalmodes-Singletenantinatrustedenvironment).
