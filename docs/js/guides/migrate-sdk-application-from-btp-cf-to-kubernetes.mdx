---
title: Migrate a JS SDK App from SAP BTP CF to Kubernetes
sidebar_label: Migrate to Kubernetes
description: Learn how to migrate your application from SAP BTP Cloud Foundry to Kubernetes.
keywords:
  - sap
  - cloud
  - sdk
...
In this how-to, we will show step by step how to migrate an application from CloudFoundry to Kubernetes.
...
- [Simple PoC Environment](#simple-poc-environments)
  - If you only do a simple PoC this kind of environment will be sufficient for you. 
...
The secret has to be in YAML format like in the example below. Use the values from service binding's JSON data to fill respective fields.
...
Now let's take a look at how to adapt our application to consume services and deploy to Kubernetes. For this jump over to the [Deploy to Kubernetes](#deploy-to-kubernetes) section.
...
To be able to use the Service Manager, you will need to have sufficient privileges in the SAP BTP Cloud Foundry. To get them to follow the steps below:
...
```
The `service-manager-credentials.txt` is sensitive data, keep it in a safe place. You will need it later on, especially the `username` and `password`.

### Broker Registration
To make the services available in the Kubernetes cluster we need to register a service broker, in the SAP BTP Cloud Foundry named `Service Manager`, to the service catalog `svcat`.

To access the `Service Manager` on BTP CF, we need to install the `service-broker-proxy` in our K8s cluster. Here's how you do it:
...
Next we need to mount this secret at a specific path for it to be usable by us, for this we follow the `xsenv` path conventions.
...
:::danger
On-Premise connectivity in Kubernetes is currently only available internally, though this will change in the near future.
This documentation is therefore only meant for internal users.
:::
...
7. For your application to reach On-Premise destinations, it needs the proxy settings, currently these have to be added manually to the secret containing the service binding.
