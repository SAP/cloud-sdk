...
Now you have to add the fields `onpremise_proxy_host` and `onpremise_proxy_port` and `url` containing the host and port, e.g. `connectivity-proxy.<namespace>`(in our case `connectivity-proxy.default`) and port, e.g. `20003`.
...
Below is an example using the SAP BTP Operator, to create a XSUAA with a very unspecific rule specifying the allowed URL with wildcards `https://*/**`, for your use case it is recommended to point directly at your URI.
...
2. Package the AppRouter as a `Dockerimage` so that it can run in Kubernetes, refer to the [documentation](https://blogs.sap.com/2020/04/03/sap-application-router/) to see how to configure an AppRouter.
   In the `xs-app.json`, the Application Router should point at the services running in your cluster, not the Ingress endpoint.
...
Notice that we mount the XSUAA the same way we do for apps using the SAP Cloud SDK.
