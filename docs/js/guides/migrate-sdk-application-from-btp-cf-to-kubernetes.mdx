...
<!-- To me it looks like the important step of containerising and deploying the app to the cluster is missing in this list -->
...
3. Finally, we adapt our application to consume services from within our Kubernetes cluster.
...
For the SDK teams we have our own docker repository that should be used: `docker-cloudsdk.docker.repositories.sap.ondemand.com`
...
Finally add annotations to your Ingress, if we use the previous example,
...
- Enable TLS for connectivitytunnel route
Write values yaml, 
- Use config guide
- Explain our example
- Use operational mode docs
- May need to add CA cert to the JVM trust store of SCC
  - Explain briefly how that is done
- Create proxy, create binding, create json secret as base64
- Create secrets for values yaml
- Add to deployment
- Add host and port to secret containing the binding