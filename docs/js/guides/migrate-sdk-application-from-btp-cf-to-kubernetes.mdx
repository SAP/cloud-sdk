...
Here's the example application we're going to use: [Kubernetes app](https://github.tools.sap/cloudsdk/k8s-e2e-app).
...
3. Create and bind the connectivity service with the `connectivty_proxy` plan.
   ...
   Additionally to binding the secret in its Kubernetes native YAML form, you need to save it as a JSON to be consumable by the connectivity proxy.
   If you use the PoC environment, save the binding as a secret, use the example below to guide you.
   Otherwise, you need to retrieve the binding first and then format it to `JSON`, then have to save it as a secret.
...
:::info
Note that we used the `stringData` instead of the default `data` field to benefit from automatic base64 encoding, instead of doing it ourselves.
...
6. Create a `values.yaml` file containing the configuration that suits your desired operational mode of the connectivity proxy, for the available operational modes refer to the [documentation](https://wiki.wdf.sap.corp/wiki/display/NGP/Operational+modes#Operationalmodes-Singletenantinatrustedenvironment).
...
7. For your application to reach On-Premise destinations, it needs the proxy settings as well as the token URL, currently, these have to be added manually to the secret containing the service binding.
...
The host has the pattern `connectivity-proxy.<namespace>`(in our case `connectivity-proxy.default`) and the port is by default `20003`, the `url` should contain the same value that `token_service_url` already does.
...
Similar to the SAP BTP CloudFoundry way of propagating a principal to handle multi-tenancy, you can also use the Application Router (AppRouter) in Kubernetes.

1. You need an XSUAA instance that can redirect to a Kubernetes URI, for this the parameter `redirect-uris` is important, however if you use one parameter while creating a service, you have to use all of them.

Below is an example using the SAP BTP Operator, to create a XSUAA with a very generic rule specifying the allowed URI with wildcards `https://*/**`, for your use case it is recommended to point directly at your URI.

If you are not using the SAP BTP Operator, consider using the same parameters just in a different format, in CloudFoundry for instance you have to provide these parameters in `JSON` format.
...
2. Package the AppRouter as a `Dockerimage` so that it can run in Kubernetes, refer to the [documentation](https://blogs.sap.com/2020/04/03/sap-application-router/) to see how to configure an AppRouter.

3. After creating a container image, create a `Deployment` and `Service` to run and expose the AppRouter, below are two examples for a AppRouter `Pod` and `Service`.
...
You can see that we mount the XSUAA the same way we do for apps using the SAP Cloud SDK.
Also, notice the way we reference the application running in our cluster, instead of an `Ingress` endpoint we directly point at the `Service`, this is possible because the AppRouter runs in our cluster and can therefore use the Kubernetes native `Service` discovery.
...
4. Finally, configure the `Ingress` to create a session-cookie that is consumed by the AppRouter and point at the AppRouter service.
   To secure your application, you can also remove all previous routes that pointed at your application endpoints and only expose them through the AppRouter, that way only users that are authenticated by your Identity Provider can access these endpoints.
   For that you just need specify the service names in your AppRouter's destinations and remove the previous rules you created for these endpoints in the `Ingress`.

   Depending on your `Ingress` controller you have to use different annotations, for the NGINX Ingress controller use the following annotations for your `Ingress`:
...
A complete example of an Ingress that only exposes the AppRouter and is using the annotations is shown in the following:
...
5. Be aware that just like in SAP BTP CloudFoundry, you have to collect the principal's JWT from the authentication header in the request when executing on of the clients, here is an example utilizing our `retrieveJwt` function:
