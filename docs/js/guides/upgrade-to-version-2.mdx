...
We have deprecated objects in version 1.X.Y and in version 2.0 they were removed.
...
In order to make the [middle ware concept](../features/odata/execute-odata-request#setting-custom-middleware) possible, it was necessary to add more state to the `requestBuillder`.
This means the `requestBuillder` is not a static method anymore but a instance method on an api object:
...
//Version 2.0.0
new MyEntityApi.requestBuillder()
```

The same is true for the static properties to build select or filters.
These are also part of an instance now inside a `schema` property:
...
//Version 2.0.0
const api = new MyEntityApi.requestBuillder()
api.requestBuilder()
      .getAll()
      .select(api.schema.FIRST_NAME, api.schema.LAST_NAME)
      .execute(destination);
```
...
//Version 2.0.0
new MyEntityApi.requestBuillder().getAll().execute({
