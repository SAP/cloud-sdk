...
Since Moment.js is a discontinued project with large size we recommend to stick with the new standard.
Note that in 2.0 version, the default remains Moment.js.
...
In order to make the [middle ware concept](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#customize-de-serialization) possible, it was necessary to add more state to the `requestBuillder`.
