...
- Change the `requestBuillder()` call as described [here](#breaking-change-on-the-requestbuilder-api).
...
In version 1.X.Y of the Cloud SDK, each entity (e.g. `BusinessPartner`) was imported directly:
...
- A new service function (e.g. `myEntityService`) provides getters for instances of all the entities' API class, which can be consumed via [JavaScript Object Destructuring](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment).
...
To make the [middle ware concept](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#customize-de-serialization) possible, it was necessary to add more state to the `requestBuillder`.
...
You can install the the generator package as a `devDependency`:

```
npm install -D @sap-cloud-sdk/generator@latest
```

or globally:

```
npm install -g @sap-cloud-sdk/generator@latest
```
