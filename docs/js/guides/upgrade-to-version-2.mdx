...
If this is not possible in your case please follow the [documentation](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#setting-moment-as-middleware) to switch back to the `moment` serialization.
...
MyEntity.builder()
  ...
  .lastName('Pan')
   .toMyRelatedEntity([
    MyRelatedEntity.builder().country('Neverland').build()
  ])
  .build();
...
The `requestBuillder` is also not a static method anymore but an instance method on an api object:
