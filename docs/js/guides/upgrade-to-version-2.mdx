...
- Change the `builder()` call as described [here](#newEntityBuilder)
...
- Change the `requestBuillder()` call as described [here](#newRequestApi)
...
If you are unsure if you have generated clients use this RegExp `class.*RequestBuilder.*extends RequestBuilder` to scan your project for RequestBuilders of a generated client.
...
- Change the used version from 1.X.Y to 2.0.0 and execute `npm install`.
- Regenerate the client with the updated generator as described [here](../features/odata/generate-odata-client).
- Change the `builder()` call as described [here](#newEntityBuilder)
- Find the invocations of the request builders.
...
- Remove `@sap-cloud-sdk/core` from the `package.json`.
...
For example, time related properties are transformed to `moment` instances.
We introduced a new [middle ware concept](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#using-custom-de-serialization) to make this more flexible.

If you would like to switch to the [temporal](https://tc39.es/proposal-temporal/docs/) serialization for date/time objects, please please follow the [documentation](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#using-temporal-middleware) and adjust your code base.
The default serialization for time related properties will be changed from `moment` to `temporal` once it reaches Stage 4.
Since `moment` is a discontinued project with large size we recommend to stick with the new standard.
Note that in 2.0 version, the default remains `moment`.
...
### <a name="newEntityBuilder"/> Breaking Change on the EntityBuilder API

For the implementation of the [middleware concept](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#using-custom-de-serialization) , two changes are introduced in version 2.0:

- Each entity now has its API class (e.g. `myEntityApi`).
- A new `service` method (e.g. `myEntityService`) provides getters for instances of all the entities' API class.
...
### <a name="newRequestApi"/> Breaking Change on the RequestBuilder API

In order to make the [middle ware concept](../features/odata/use-odata-v2-type-safe-client-for-javascript-typescript#using-custom-de-serialization) possible, it was necessary to add more state to the `requestBuillder`.
...
The `requestBuillder` is also not a static method anymore but an instance method of an API object:
...
- The property `userJwt` is rename to just `jwt`.

A typical API adjustment would look like:
...
- `@sap-cloud-sdk/odata-v4` which contains the OData functionality specific to OData version 4.
