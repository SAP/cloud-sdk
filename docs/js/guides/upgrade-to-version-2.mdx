...
Each entity now has its API class (e.g. `myEntityApi`). A new `service` method (e.g. `myEntityService`) provides getters for instances of all the entities' API class.
...
//Version 2.0.0
const { myEntityApi } = myEntityService();
myEntityApi.requestBuillder().getAll().execute({
