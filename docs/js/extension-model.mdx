...
SAP Cloud SDK for JavaScript welcomes extensions that bring value to SDK users by simplifying and/or improving cloud application development within the SAP ecosystem.
The contributed feature or API library becomes an extension of SDK and is released under the SAP Cloud SDK umbrella into the `@sap-cloud-sdk` scope in [npmjs.com](https://www.npmjs.com/).
...
If this is not possible, e.g. For legal reasons, licensing has to be clarified in person.
...
Final approval is on the SDK team.
...
- Contribution team should use coding conventions of the SDK unless it's explicitly approved to be otherwise
...
- After reaching `Beta`, features or API libraries have to be documented by the contribution team on SDK's documentation portal
...
- The contribution team has to set on a regular release cycle.
  SDK uses 2 weeks released cycle and it's preferred regularity.
