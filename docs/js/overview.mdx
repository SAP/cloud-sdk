...
| Operates dynamically on the metadata and creates an on-the-fly-client. | Uses pregenerated client at runtime. For many standard systems like SAP S/4HANA, clients are available via `npm` or can be generated with the [generator](features/odata/generate-odata-client.mdx). |
