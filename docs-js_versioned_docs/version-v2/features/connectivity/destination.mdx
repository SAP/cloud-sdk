...
`myDestination` is an SAP Cloud SDK representation of a [destination](pathname:///api/v2/interfaces/sap_cloud_sdk_connectivity.Destination.html).
...
The value of the environment variable is expected to be a stringified JSON array, where the items adhere to the [Destination interface](pathname:///api/v2/interfaces/sap_cloud_sdk_connectivity.Destination.html).
...
It takes a [destination object](pathname:///api/v2/interfaces/sap_cloud_sdk_connectivity.Destination.html), transforms it to a JSON object, and adds it to `process.env.destinations`.
...
If you need the destination for another custom service, create a function with [ServiceBindingTransformFunction type](pathname:///api/v2/types/sap_cloud_sdk_connectivity.ServiceBindingTransformFunction.html) to transform the [Service interface](pathname:///api/v2/interfaces/sap_cloud_sdk_connectivity.Service.html) into the [Destination interface](pathname:///api/v2/interfaces/sap_cloud_sdk_connectivity.Destination.html).
...
If you want to consider only the service bindings, call the [destinationForServiceBinding()](pathname:///api/v2/functions/sap_cloud_sdk_connectivity.destinationForServiceBinding.html) function with the service name and options.
