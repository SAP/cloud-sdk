...
- For OData clients: This release changed the default HTTP response lifecycle for OData request-execution results.
  ...
  While this is considered a fix and accpetable change for the expert _OData Generic Client_, we unintentionally changed the behavior in the convenience classes for OData v2 and OData v4 as well.
