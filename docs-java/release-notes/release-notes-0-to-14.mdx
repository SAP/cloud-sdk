...
- We are planning to discontinue support of Business Logging APIs with SAP Cloud SDK v5 by end of the year.
  ...
  The code can now be generated instead by using the [OData](/features/odata/vdm-generator) and [OpenAPI](/cloud-sdk/docs/java/features/rest/generate-rest-client) generators with the `EDMX` and `JSON` specifications of the service.
