...
- Improved the upgrade path from SAP Cloud SDK version 4 by gracefully handling older implementations of `Resilience4jDecorationStrategy`.
  ...
  This is no longer required, as the SAP Cloud SDK 4 strategy will gracefully be ignored, if there is exactly one alternative.
