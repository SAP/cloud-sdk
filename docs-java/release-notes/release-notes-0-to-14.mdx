...
- We are planning to discontinue support of Business Logging APIs with SAP Cloud SDK v5 by end of the year.
  ...
  - **BTP Business Logging - All**
    - **BTP Business Logging OData**: `com.sap.cloud.sdk.services.scp.businesslogging.odata.*`
      - The replacement is to generate the API yourself using the existing plugins found in the [deprecated pom file](services/scp-business-logging-odata/pom.xml) with the `JSON` and `EDMX` spec files.
    - **BTP Business Logging REST**: `com.sap.cloud.sdk.services.scp.businesslogging.rest.*`
      - The replacement is to generate the API yourself using the existing plugins found in the [deprecated pom file](services/scp-business-logging-rest/pom.xml) with the `YAML` spec files.
