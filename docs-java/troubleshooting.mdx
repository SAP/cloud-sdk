...
- The SAP Cloud SDK’s default `FORWARD_USER_TOKEN` strategy forwards the _entire_ token with every group claim—in the `Authorization` header during destination lookup. Large tokens can exceed the platform’s header-size limit, triggering the 431.

  You may try out other strategies, such as the now-deprecated `LOOKUP_THEN_EXCHANGE` which can appropriately exchange for a more compact token at the cost of performance overhead.
