...
- The SAP Cloud SDK’s default `FORWARD_USER_TOKEN` strategy forwards the _entire_ token, including all group claims, in the `Authorization` header when performing a destination lookup. If the token is too large, it can exceed the platform’s header size limit and trigger an HTTP 431 error.
