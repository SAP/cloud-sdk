...
- The SDK’s default `FORWARD_USER_TOKEN` strategy forwards the _entire_ token with every group claim—in the `Authorization` header during destination lookup. Large tokens can exceed the platform’s header-size limit, triggering the 431.
