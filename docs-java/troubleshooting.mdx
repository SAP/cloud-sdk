...
- You are caching the `Destination` instance in your application code instead of retrieving it for each request.
  Since the `Destination` object stores the JWT needed for passing the SAP Cloud Connector, the JWT does not get refreshed after its expiration.

  To solve this issue, retrieve the `Destination` for each request again with the `DestinationAccessor` API.
