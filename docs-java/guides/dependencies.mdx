...
<dependencyManagement>
    <dependencies>
        <dependency>
            ...
            <artifactId>cf-tomee7-bom</artifactId><!--This might change-->
            <version>use-latest-version-here</version>
            <type>pom</type>
            <scope>import</scope>
        </dependency>
...
From now on, whenever you switch to a newer version of the SJB, it might not be magically compatible with the SAP CLoud SDK.
...
To prevent any SAP Java Buildpack issues we recommend using the `cf-tomee7-bom`#This might change
