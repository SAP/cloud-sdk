...
Alternatively, you can use [BOMs provided by the SAP Java Buildpack](https://help.sap.com/docs/BTP/65de2977205c403bbc107264b8eccf4b/6c6936e8e4ea40c9a9a69f6783b1e978.html)
directly, [as described here](#the-sap-cloud-sdk-bill-of-material).
:::

:::caution Compatible Java and Spring Versions

Please note that the SAP Java Buildpack is only compatible with version 7 of the Java EE and the `javax.*` namespaace.
Consequently, frameworks based on the newer Jakarta EE and the `jakarta.*` namespace are not supported.

In particular, Spring 6.X, Spring Boot 3.X and Java 17 are not compatible with the SJB.
:::
