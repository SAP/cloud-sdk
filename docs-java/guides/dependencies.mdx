...
{ label: 'Tomcat with SAP Java Buildpack', value: 'SJB', }
]}>
...
:::caution Dedicated BOM for SAP Java Buildpack
Please refer to the [buildpack related notes below](#notes-on-the-sap-java-buildpack) in case you are using the SAP Java Buildpack with a WAR based deployment.
:::
...
Alternatively, you can use the [BOMs provided by the SAP Java Buildpack](https://help.sap.com/docs/BTP/65de2977205c403bbc107264b8eccf4b/6c6936e8e4ea40c9a9a69f6783b1e978.html) directly.
...
Using our dedicated buildpack BOM `com.sap.cloud.sdk:sdk-sjb-bom` helps preventing scope conflicts.
...
It will set the version and the scope of the dependencies the SAP Java Buildpack adds to your application at runtime.
...
When you use the dedicated `sdk-sjb-bom` or any of the [BOMs provided by the SAP Java Buildpack directly](https://search.maven.org/search?q=com.sap.cloud.sjb.cf) the `token-client` should have the scope `provided`.
...
This replaces our `sdk-sjb-bom` (a convenience combination of the SAP Java Buildpack and our SAP Cloud SDK BOMs) with our `sdk-bom`.
...
SAP Cloud SDK can be used with JakartaEE (e.g. via Spring Boot 3, Spring 6, and Java 17.) thank to the experimental module `servlet-jakarta`.
Here are instructions to update your application:
...
3. Replace any `javax` dependency with a `jakarta` dependency:
...
4. Modify the existing `scp-cf` dependency to enable the new `servlet-jakarta` module:
...
5. Remove outdated dependencies:

- Remove `logback-classic`
- Some dependencies such as the `XSUAA Security library` are not yet updated for Spring Boot 3
