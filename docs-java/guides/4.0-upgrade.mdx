---
id: 4.0-upgrade
title: Update to Version 4.0 of the SAP Cloud SDK
hide_title: false
hide_table_of_contents: false
sidebar_label: Update to Version 4.0
description: Update your application to use version 4.0 of the SAP Cloud SDK
keywords:
  - sap
  - cloud
  - sdk
...
Finally, version 4.0 discontinued some (legacy) modules and plugins.
Most notably are:
...
With version 4.0 we also reduce the amount of dependencies the SAP Cloud SDK brings.
...
With version 4.0 you can simplify your code for running asynchronous tasks with the newly introduced `ThreadContextExecutors`:

```java
Future runningTask = ThreadContextExecutors.submit(() -> operation());
```

The new API also integrates conveniently with CAP and can be integrated to work with Springs `@Async`.
