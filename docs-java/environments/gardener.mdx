...
It will create a Kubernetes resource in your cluster, allowing you to create requests against arbitrary destinations.
...
This will target the Kubernetes resource `dynamic-destination` we created above which will be resolved to the destination name we passed in the header.
