...
Given you have created a destination with the name `my-destination` in the BTP Cockpit, create the following YAML file:
...
This will create a Kubernetes resource in your cluster, pointing to the destination `my-destination` you created in the BTP cockpit.
...
Apply the YAML with `kubectl apply -n` into the Transparent Proxy Kubernetes namespace to take effect.
