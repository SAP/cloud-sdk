...
If you are re-using the same HTTP
...
If all responses in a `BatchResponse` is evaluated using `getReadResult(...)` or/and `get(index)`, this ensures that the underlying `InputStream` of the `HttpEntity` is fully consumed
