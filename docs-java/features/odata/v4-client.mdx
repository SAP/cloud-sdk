...
If all responses in a `BatchResponse` is evaluated using `getReadResult(...)` or/and `getModificationResult(...)`, this ensures that the underlying `InputStream` of the `HttpEntity` is fully consumed
