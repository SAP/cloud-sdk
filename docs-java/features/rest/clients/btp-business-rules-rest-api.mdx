...
You can check the SAP BTP Business Rules API's model definition on the [SAP Business Accelerator Hub](https://api.sap.com/package/SAPCPBusinessRulesAPIs?section=Artifacts) or use your IDE to discover available properties via its auto-complete function.
