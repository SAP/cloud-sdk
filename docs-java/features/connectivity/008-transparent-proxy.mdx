...
The provider tenant identifier is especially useful in contexts where:

- The transparent proxy operates in environments where tenant information is not readily available
- You need a default tenant for authentication and authorization purposes
- You want to ensure consistent tenant resolution during destination preparation
...
DestinationOptions options = DestinationOptions.builder()
    .parameter("x-tenant-id", "my-tenant-id")
    .parameter("x-fragment-name", "my-fragment")
    .build();
...
The following parameters can be used with `DestinationOptions.builder().parameter(key, value)`:
...
- **`x-fragment-name`**: Specifies the name of a fragment to apply to the destination configuration
  ...
  ```java
  .parameter("x-fragment-name", "production-fragment")
  ```

- **`x-fragment-optional`**: Controls whether the fragment is optional (default: false).
  Set to "true" or "false"
  ```java
  .parameter("x-fragment-optional", "true")
  ```

##### Tenant Configuration

- **`x-tenant-id`**: Specifies the tenant ID for the destination lookup.
  Overrides the current tenant from context

  ```java
  .parameter("x-tenant-id", "tenant-123")
  ```

- **`x-tenant-subdomain`**: Specifies the tenant subdomain.
  Cannot be used together with `x-tenant-id`

  ```java
  .parameter("x-tenant-subdomain", "my-subdomain")
  ```

- **`x-token-service-tenant`**: Specifies the tenant for token service calls
  ```java
  .parameter("x-token-service-tenant", "token-tenant")
  ```

##### Authentication Configuration

- **`authorization`**: Provides a custom authorization header value (e.g., Bearer token)

  ```java
  .parameter("authorization", "Bearer my-custom-token")
  ```

- **`x-client-assertion`**: Provides a client assertion for OAuth flows

  ```java
  .parameter("x-client-assertion", "eyJhbGciOiJSUzI1NiIs...")
  ```

- **`x-client-assertion-type`**: Specifies the client assertion type

  ```java
  .parameter("x-client-assertion-type",
             "urn:ietf:params:oauth:client-assertion-type:jwt-bearer")
  ```

- **`x-client-assertion-destination-name`**: References another destination for client assertion
  ```java
  .parameter("x-client-assertion-destination-name",
...
- **`x-subject-token-type`**: Specifies the subject token type for token exchange
  ...
  ```java
  .parameter("x-subject-token-type",
...
- **`x-actor-token`**: Provides an actor token for token exchange flows
  ...
  ```java
  .parameter("x-actor-token", "actor-token-value")
  ```

- **`x-actor-token-type`**: Specifies the actor token type
  ```java
  .parameter("x-actor-token-type",
             "urn:ietf:params:oauth:token-type:jwt")
  ```
...
DestinationOptions options = DestinationOptions.builder()
    // Fragment configuration
    .parameter("x-fragment-name", "production-fragment")
    .parameter("x-fragment-optional", "false")

    // Tenant configuration
    .parameter("x-tenant-id", "tenant-123")
    .parameter("x-token-service-tenant", "service-tenant")

    // Authentication
    .parameter("authorization", "Bearer custom-token")
