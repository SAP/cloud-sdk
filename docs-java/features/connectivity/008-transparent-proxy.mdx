...
DestinationOptions options = DestinationServiceOptionsAugmenter.augmenter()
    .customHeaders(new Header("x-tenant-id", "my-tenant-id"))
    .customHeaders(new Header("x-fragment-name", "my-fragment"))
    .build();
...
- **`x-tenant-subdomain`**: Specifies the tenant subdomain.
  ...
  ```java
  .customHeaders(new Header("x-tenant-subdomain", "my-subdomain"))
  ```

- **`x-token-service-tenant`**: Specifies the tenant for token service calls
  ```java
  .customHeaders(new Header("x-token-service-tenant", "token-tenant"))
  ```
...
- **`authorization`**: Provides a custom authorization header value (e.g., Bearer token)
  ...
  ```java
  .customHeaders(new Header("authorization", "Bearer my-custom-token"))
  ```
