...
Explore the [BtpServiceOptions](pathname:///java-api/v5/com/sap/cloud/sdk/cloudplatform/connectivity/BtpServiceOptions.html) class to find any options relevant for your service.
...
The SAP Cloud SDK supports the credential types `binding-secret` and `X509_GENERATED` for IAS service bindings.

<!-- Prepared text, to be enabled with https://github.com/SAP/cloud-sdk/pull/1756
...
The types `X509_PROVIDED` and `X509_ATTESTED` are currently not supported.
:::
