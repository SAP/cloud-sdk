...
For BTP services that offer different APIs (such as the Workflow service) you should also additionally define which API the resulting `HttpDestination` should point to using `BtpServiceOptions`.
