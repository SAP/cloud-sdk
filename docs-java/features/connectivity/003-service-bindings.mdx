...
[_Service Bindings_](https://github.com/SAP/btp-environment-variable-access/wiki/Fundamentals#service-binding) provide an application with the necessary credentials to connect to a service when deployed on the SAP Business Technology Platform (BTP).
...
Explore the `BtpServiceOptions` class to find any options relevant for your service.
...
- For IAS-based services it is sufficient to conform to the [dedicated service binding format](#ias-service-binding-format) for IAS.
...
The SAP Cloud SDK supports the credential types `binding-secret` as well as `X509_GENERATED` and `X509_ATTESTED` for IAS service bindings.

If you want to use the `X509_ATTESTED` credential type, you need to add the `connectivity-ztis` dependency to your project.
Read more about how to confiugre your app for this credential type on the documentation for [using certificates from the Zero Trust Identity Service (ZTIS)](/docs/java/features/connectivity/mtls#using-automated-certificate-rotation-using-the-zero-trust-identity-service-sap-internal).
...
If your service is secured using IAS and is using the [dedicated service binding format](#ias-service-binding-format) supported by the SAP Cloud SDK, you can obtain a destination by passing the service label as the `ServiceIdentifier`:
...
In case the service does not require a JWT token (e.g. the Event Broker service) you can pass the additional option `IasOptions.withoutTokenForTechnicalProviderUser()`.
Together with setting `OnBehalfOf.TECHNICAL_USER_PROVIDER` this will skip retrieving a token from IAS and only sending the X.509 certificate, saving a bit of performance.
...
- The `credentials` section must contain an object `endpoints` which must contain at least one entry.
  - Each entry under `endpoints` must be an object containting a `uri` field set to a string.
...
Services backed by XSUAA typically have very similar service binding formats that differ only slightly.
...
However, in some cases the SAP Cloud SDK or other frameworks may implcitly access service bindings.
...
var newAccessor = new ServiceBindingMerger(List.of(accessor, () -> binding), ServiceBindingMerger.KEEP_EVERYTHING);
