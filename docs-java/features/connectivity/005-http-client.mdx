...
```java
ApacheHttpClient5Cache cache = ApacheHttpClient5CacheBuilder.duration(Duration.ofHours(1)).build();
...
Once a new client is created (for a destination) cookies from previous requests will no longer be avaiable.
