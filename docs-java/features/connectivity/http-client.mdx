...
### Set up a `com.sap.cloud.security.client.HttpClientFactory`

The exception `More than 1 Custom HttpClientFactory service provider found. There should be only one` might be encountered.
The solution is in the [FAQ](../../faq.mdx#im-observing-a-more-than-1-custom-httpclientfactory-service-provider-found-there-should-be-only-one-exception)
