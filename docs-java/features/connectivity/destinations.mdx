...
For better performance, while querying for destinations with user propagation, additionally also provide the `EXCHANGE_ONLY` token exchange strategy.Refer [here](#token-exchange-options) for more details.
