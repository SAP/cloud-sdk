...
:::note Caching
Please note that these destinations are currently not cached.
Please consider re-using the obtained destination objects when using `OAuth2ClientCredentials` or `OAuth2UserTokenExchange` to avoid unnecessary token retrievals.
:::
