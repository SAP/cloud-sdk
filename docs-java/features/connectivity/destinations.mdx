...
By default, the SAP Cloud SDK uses the `RequestScopedHttpClientCache` to store `HttpClient` instances during a specific (incoming) request.
...
Or by manually providing your [`VCAP_SERVIES`](#create-and-provide-vcap_services-variable) variable.
