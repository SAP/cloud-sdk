...
:::caution Shared Cache Size

The cache size is shared between all tenants and principals.
That means that one tenant (with a lot of principals) might take up the entire cache.

For example, if a thousand principals of the same tenant all query a single destination, the cache will be entirely filled up with destinations belonging to the same tenant.
...
Therefore, we **strongly recommend** adjusting the cache **once at application startup** only.
