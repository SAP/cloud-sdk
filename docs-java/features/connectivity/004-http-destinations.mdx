...
In most cases, however, the BTP Destination Service will [provide a `Authorization` header](btp-destination-service#how-authentication-at-the-target-system-works), so this step will be skipped.
...
Adding this header directly to the existing `Destination` instance, however, is not possible, as [the `Destination` interface is immutable](#about-immutability).
