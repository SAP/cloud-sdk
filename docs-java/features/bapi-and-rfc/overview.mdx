...
:::danger Warning
`RFC` and `SOAP` modules are **deprecated**.
These modules will be kept for version 5 as well, but will be removed in the future.
Please consider searching for an OData based service (e.g., via SAP Business Accelerator Hub) as possible replacement.
The SAP Cloud SDK also offers OData type safe clients (both OData v2 and v4).
...
If you're starting from scratch, generate a `Spring Boot` project following the steps in the [Getting started](../../getting-started#generating-a-project-from-a-maven-archetype) guide.
:::
...
```bash
export destinations='[{"name": "destinationName", "type": "RFC"}]'
```
