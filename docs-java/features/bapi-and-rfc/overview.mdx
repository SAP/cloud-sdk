...
:::danger Warning
`RFC`, `JCo` and `SOAP` modules are **deprecated** and will be removed in version 5 of the SAP Cloud SDK.
After that, you can still use our SDK version 4, as it will be maintained for another half a year (until around Q1 2024).
Please consider searching for an OData based service (e.g., via SAP API Business Hub) as possible replacement, and SDK offers OData type safe clients (both OData v2 and v4).
:::
