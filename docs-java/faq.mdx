...
:::caution Compatibility of OpenAPI Features and `RestTemplate`

Due to an [incompatible change in the Spring Framework 6](https://github.com/spring-projects/spring-framework/wiki/Upgrading-to-Spring-Framework-6.x#web-applications) the OpenAPI related functionality of the SAP Cloud SDK is currently not compatible with **version 6 of the Spring Framework**.

The issue is caused by a change to the `RestTemplate`, which now requires Apache Http Client version 5 (previously required version 4).
...
This **does not** affect use cases where `RestTemplate` is used directly with the [Http Client provided by the SAP Cloud SDK](/docs/java/features/connectivity/http-client).
...
- [Configure SAP Java Buildpack to run with Java 17](https://help.sap.com/docs/btp/sap-business-technology-platform/sapmachine#activation)
  ...
  ```
- Please note, some dependencies such as the `XSUAA Security library` are not yet fully compatible for Java17 or Spring Boot 3.
...
:::warning warning
All the NEO specific classes and functions have been deprecated.
